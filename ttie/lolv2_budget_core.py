"""T022-D single dark-EV-bound variant; low-only native-resolution execution."""
import argparse
import json
import time
...
def low_image_opener(allowed,opened):
    ...
    def guarded(path,*args,**kwargs):
        if str(Path(path).resolve()) not in allowed:
            raise PermissionError('T022-D accepts only frozen low-light images')
...
def main():
    ...
    config=dict(task='T022-D',method='T014 Region2 dark EV upper2',dark_ev_upper=2.,assets=assets,
