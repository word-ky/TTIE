"""T023-A single dark-EV-bound variant; low-only native-resolution execution."""
import argparse
import json
import time
...
def low_image_opener(allowed,opened):
    ...
    def guarded(path,*args,**kwargs):
        if str(Path(path).resolve()) not in allowed:
            raise PermissionError('T023-A accepts only frozen low-light images')
...
def main():
    ...
    config=dict(task='T023-A',method='T014 Region2 dark EV upper2',dark_ev_upper=2.,assets=assets,
    ...
    write(args.out/'freeze.json',dict(completed_utc=datetime.now(timezone.utc).isoformat(),
          rows=rows,opened_images=opened,normal_images_available=False,low_only_decoder_enforced=True,
