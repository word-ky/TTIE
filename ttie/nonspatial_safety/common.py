...
LOCK = Path('research_log/T020A_pipeline_lock.json')
...
def preparation_bindings(cohort):
    ...
    paths=dict(exclusions_sha256=Path('research_log/T020A_exclusions.json'),
...
def preflight(source):
    lock = read(LOCK)
    new = [f'ttie/nonspatial_safety/{n}.py' for n in ('__init__','common','prepare','select','predict','evaluate')]
    files = list(lock['baseline_code_sha256']) + new + [LOCK.as_posix(), 'research_log/T020A_exclusions.json', 'scripts/run_t020a_a6000.sh', 'research_log/T020A_verify.py']
