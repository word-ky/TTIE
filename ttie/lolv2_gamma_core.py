"""T026-A single dark-EV-bound variant; low-only native-resolution execution."""
import argparse
import json
import time
...
def low_image_opener(allowed,opened):
    ...
    def guarded(path,*args,**kwargs):
        if str(Path(path).resolve()) not in allowed:
            raise PermissionError('T026-A accepts only frozen low-light images')
...
def main():
    ...
    config=dict(task='T026-A',method='T014 Region2 dark EV upper2 active gamma lower0.5',dark_ev_upper=2.,active_gamma_lower=.5,assets=assets,
