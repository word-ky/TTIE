...
from .common import CROSS, read, write, sha, now, tensor_sha, preflight, preparation_bindings
...
def select(cohort, output, source, device):
    lock, code = preflight(source); cohort=cohort.resolve(); bindings=preparation_bindings(cohort)
    ...
    write(output/'config.json',dict(task='T019-D',source_sha=source,source_code_sha256=code,pipeline_lock_sha256=sha(Path('research_log/T019D_pipeline_lock.json')),
        selector_receipt_sha256=lock['selector_receipt_sha256'],**bindings,
