"""T022-A native-resolution low-only execution of the unchanged T014 core."""
import argparse
import json
import time
...
def main():
    ...
    # normal images are not deployed to the server until this run has finished.
    opened=[]; original_open=Image.open
    def recorded_open(path, *a, **kw):
        opened.append(str(Path(path).resolve()))
        return original_open(path,*a,**kw)
    Image.open=recorded_open
    files=assets['files']
    ...
    config=dict(task='T022-A',method='T014 Sobolev Region2',assets=assets,
                split_sha256=sha(args.split),seed=7,tf32=False,max_steps=40,
    ...
    for i,row in enumerate(split['selected']):
        ...
        result,t,decision=trajectory(image,scorer,receipt,head,basis='region2',max_steps=40)
        torch.cuda.synchronize(); seconds=time.perf_counter()-start
        assert torch.isfinite(result['image']).all()
        directory=args.out/f'{i:03d}'; directory.mkdir()
        torch.save({k:result[k] for k in ['image','raw','grid']},directory/'output.pt')
        ...
        files_saved={name:dict(sha256=sha(directory/name),bytes=(directory/name).stat().st_size)
    ...
    write(args.out/'freeze.json',dict(completed_utc=datetime.now(timezone.utc).isoformat(),
          rows=rows,opened_images=opened,normal_images_available=False,
