...
def summarize(rows):
    ...
    return dict(groups=groups,clauses=clauses,passed=sum(clauses.values()),fresh_qualified=all(clauses.values()),
        verdict='T018-E fresh qualification positive' if all(clauses.values()) else 'T018-E fresh qualification negative')


@torch.no_grad()
def evaluate(cohort,selected,images,output,source,device):
...
if __name__=='__main__':
    ...
    for k in ('cohort','selected','images','output'):p.add_argument('--'+k,type=Path,required=True)
    p.add_argument('--source-sha',required=True);p.add_argument('--device',default='cuda:0');a=p.parse_args()
    evaluate(a.cohort,a.selected,a.images,a.output,a.source_sha,a.device)
