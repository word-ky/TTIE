...
def freeze_manifest(images, output, source):
    lock, code = preflight(source); prior = read('research_log/T018E_exclusions.json')
    excluded = set(prior['excluded_ids'])
    manifest = evaluation_manifest(images, excluded, count=40, split='qualification_t018e')
    ...
    write(output/'manifest_frozen.json', dict(finalized_utc=now(), manifest_sha256=sha(output/'manifest.json'),
        exclusions_sha256=sha(Path('research_log/T018E_exclusions.json')), source_sha=source, source_code_sha256=code,
