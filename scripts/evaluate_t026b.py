...
def main():
    ...
    for r,s in zip(frozen['rows'],split['selected']):
        ...
        assert r['updates']==(80 if any(d['gate']['active']) else 0)
    rows=[];errors=[]
    for r,s in zip(frozen['rows'],split['selected']):
    ...
    summary=dict(rows=100,ssim=METRIC,psnr='Per-image -10log10 RGB MSE, float64 arithmetic over float32 [0,1] pixels, no crop/resize',
                 ...
                 total_updates=sum(r['updates'] for r in rows),official_test_evaluated=False,verdict='metrics-computed-pending-T026B-joint-gate')
