...
from ttie.support_trust import select,bind_radius
import sys
...
def main():
    ...
    for k in ['low-root','manifest','assets','spec','radius','out']:p.add_argument('--'+k,type=Path,required=True)
    a=p.parse_args();torch.manual_seed(7);torch.set_num_threads(1)
    torch.use_deterministic_algorithms(False)
    torch.backends.cuda.matmul.allow_tf32=False;torch.backends.cudnn.allow_tf32=False
    assert torch.cuda.is_available()
    SPEC=json.loads(a.spec.read_bytes());radius_receipt=json.loads((a.radius/'freeze.json').read_bytes())
    radius=bind_radius(radius_receipt,SPEC,sha(a.radius/'freeze.json'))
    assert sha(a.radius/'source.pt')==radius_receipt['source_sha256']
    source=torch.load(a.radius/'source.pt',weights_only=True,map_location='cuda:0')
    ...
    assert files['energy']['sha256']==radius_receipt['checkpoint_sha256']
    ...
    write(a.out/'config.json',dict(task='T032-A',assets=assets,spec=SPEC,spec_sha256=sha(a.spec),
    ...
    for i,r in enumerate(rows):
        ...
        values=distances(source['source_features'],t['features'].cuda(),source['x_mean'],source['x_scale']).cpu().tolist()
        decision=select(original_decision['scores'],values,radius)
        ...
        assert all(torch.isfinite(x).all() for x in [t['states'],t['images'],torch.tensor(values)])
        ...
        torch.save(dict(states=t['states'],grids=t['grids'],features=t['features'],distances=torch.tensor(values,dtype=torch.float64)),d/'trajectory.pt')
        ...
        frozen.append(dict(index=i,low=r['low'],updates=t['diagnostics']['steps'],states=len(t['states']),
            original_step=decision['original_step'],selected_step=step,cutoff=decision['cutoff'],crossing=decision['exit_step'],fallback=None,
            trajectory_seconds=trajectory_seconds,guard_seconds=guard_seconds,original_distance=values[decision['original_step']],selected_distance=values[step],persisted_utc=utc(),
    ...
    write(a.out/'freeze.json',dict(completed_utc=utc(),rows=frozen,opened_images=opened,normal_decodes=0,
        manifest_sha256=sha(a.manifest),spec_sha256=sha(a.spec),radius_freeze_sha256=sha(a.radius/'freeze.json'),config_sha256=sha(a.out/'config.json'),assets_unchanged=True))
