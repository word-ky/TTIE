...
manifest=research_log/T012_source_manifest.json
extra=()
if [ "$stage" = B ]; then
    manifest=research_log/T012_manifest.json
    extra=(--head research_log/T012_head.pt --stopping-receipt research_log/T012_stopping_receipt.json)
fi
python -m ttie.stop_pilot --stage "$stage" --source-sha "$source_sha" \
  --manifest "$manifest" --source-manifest research_log/T012_source_manifest.json \
