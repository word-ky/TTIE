...
def main():
    ...
    for r,s,o in zip(f['rows'],split,ours):
        ...
        rows.append(dict(index=r['index'],low=s['low'],retinex_psnr=psnr,retinex_ssim=ssim,ours_psnr=float(o['ours_psnr']),ours_ssim=float(o['ours_ssim']),
    ...
    summary=dict(classification='benchmark complete',metrics={k:stats([r[k] for r in rows]) for k in ['retinex_psnr','retinex_ssim','ours_psnr','ours_ssim','delta_psnr','delta_ssim']},
