...
def main(current,baseline):
    ...
    aggregates={}
    for name,table,prefix in [('raw',rows,'raw'),('T022C',old,'ours'),('T023A',rows,'ours')]:
        aggregates[name]={}
    ...
    positive=deltas['psnr']['mean']>=.5 and aggregates['T023A']['ssim']['mean']>=aggregates['T022C']['ssim']['mean']
    ...
    summary=dict(status='experiment-complete',variant='accepted C with source-recalibrated energy only',rows=100,aggregates=aggregates,paired_deltas=deltas,
                 selected_step_histogram=np.bincount([r['selected_step'] for r in delta_rows],minlength=41).tolist(),
                 selected_step40=sum(r['selected_step']==40 for r in delta_rows),
                 ...
                 gate=dict(psnr_delta_threshold=.5,ssim_must_not_decrease=True,psnr_pass=deltas['psnr']['mean']>=.5,ssim_pass=aggregates['T023A']['ssim']['mean']>=aggregates['T022C']['ssim']['mean']),
