"""Audit wrapper around the unchanged accepted T027-A exporter main."""
...
from ttie import retinex_exporter
...
def main():
    ...
    assert Path(b['checkpoint']).stat().st_size==6478393
    ...
    (a.out/'config.json').write_text(json.dumps(dict(task='T033-A',started_utc=utc(),binding=b,binding_sha256=sha(a.binding),
        upstream_commit='1e9a0efce4b306b6701b824768370ff26066c32a',GT_mean=False,self_ensemble=False,torch=torch.__version__,cuda=torch.version.cuda,gpu=torch.cuda.get_device_name()),indent=2)+'\n')
    sys.argv=['retinex_exporter','--low',*[str(p) for p in paths],'--checkpoint',b['checkpoint'],'--config',b['config'],'--out',str(a.out/'outputs')]
    retinex_exporter.main()
    receipt=json.loads((a.out/'outputs/receipt.json').read_bytes());assert receipt['GT_mean'] is False and receipt['self_ensemble'] is False
