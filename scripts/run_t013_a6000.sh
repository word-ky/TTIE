...
manifest=research_log/T013_source_manifest.json
extra=()
if [ "$stage" = B ]; then
    manifest=research_log/T013_manifest.json
    extra=(--energy research_log/T013_energy.pt --energy-receipt research_log/T013_energy_receipt.json)
fi
python -m ttie.energy_pilot --stage "$stage" --source-sha "$source_sha" \
  --manifest "$manifest" --source-manifest research_log/T013_source_manifest.json \
