...
def main(current,baseline):
    ...
    for r,a,f in zip(rows,old,freeze['rows']):
        ...
        assert d['diagnostics']['steps']==40
        box=d['diagnostics']['action_box'];before=prior['diagnostics']['action_box']
        lower_expected=np.array(before['lower']);active=np.array(d['gate']['active']).reshape(2,2)
        lower_expected[0,1][active]=.5
        assert box['upper']==before['upper'] and np.array_equal(lower_expected,np.array(box['lower']))
        ...
        assert trajectory['states'].shape[0]==41 and all(torch.isfinite(v).all() for v in trajectory.values())
    ...
    aggregates={}
    for name,table,prefix in [('raw',rows,'raw'),('T022C',old,'ours'),('T026A',rows,'ours')]:
        aggregates[name]={}
    ...
    positive=deltas['psnr']['mean']>=.5 and aggregates['T026A']['ssim']['mean']>=.3282314776612914-1e-12
    independent_positive=statistics.mean(r['delta_psnr'] for r in delta_rows)>=.5 and statistics.mean(float(r['ours_ssim']) for r in rows)>=.3282314776612914-1e-12
    assert positive==independent_positive
    times=[float(r['seconds']) for r in rows]
    summary=dict(status='experiment-complete',variant='active gamma lower0.8->0.5 only',rows=100,aggregates=aggregates,paired_deltas=deltas,
                 selected_step_histogram=np.bincount([r['selected_step'] for r in delta_rows],minlength=41).tolist(),
                 ...
                 gate=dict(psnr_delta_threshold=.5,ssim_must_not_decrease=True,ssim_floor=.3282314776612914,ssim_tolerance=1e-12,psnr_pass=deltas['psnr']['mean']>=.5,ssim_pass=aggregates['T026A']['ssim']['mean']>=.3282314776612914-1e-12),
    ...
    verification=dict(passed=True,all100_gates_identical_to_C=True,all100_only_active_gamma_lower_changed=True,
