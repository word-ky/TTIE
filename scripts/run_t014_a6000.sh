...
manifest=research_log/T014_source_manifest.json
extra=()
if [ "$stage" = B ]; then
    manifest=research_log/T014_manifest.json
    extra=(--energy research_log/T014_energy.pt --control research_log/T014_value_only.pt --energy-receipt research_log/T014_energy_receipt.json)
fi
python -m ttie.sobolev_pilot --stage "$stage" --source-sha "$source_sha" \
  --manifest "$manifest" --source-manifest research_log/T014_source_manifest.json \
  ...
  --receipt research_log/T007_joint_calibration.json --t006-images /home/wenchang/asdasdsad/wjq/TTIE/shared/t006/images \
  --output "$AUTODL_ARTIFACTS_DIR/audit" --device cuda:0 "${extra[@]}"
