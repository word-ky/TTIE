"""Post-freeze comparison to accepted T022-A; fixed gate and independent checks."""
import csv
import hashlib
import json
import math
...
def main(current,baseline):
    ...
    for r,a,f in zip(rows,old,freeze['rows']):
        ...
        upper=np.array(before['upper']);dark=np.array(d['gate']['active']) & (np.array(d['gate']['winner'])==0)
        upper[0,0][dark.reshape(2,2)]=2.
        assert box['lower']==before['lower'] and np.array_equal(upper,np.array(box['upper']))
    ...
    aggregates={}
    for name,table,prefix in [('raw',rows,'raw'),('T022A',old,'ours'),('T022C',rows,'ours')]:
        aggregates[name]={}
    ...
    positive=deltas['psnr']['mean']>=.5 and aggregates['T022C']['ssim']['mean']>=aggregates['T022A']['ssim']['mean']
    ...
    summary=dict(status='experiment-complete',variant='active dark EV upper2 only',rows=100,aggregates=aggregates,paired_deltas=deltas,
                 ...
                 gate=dict(psnr_delta_threshold=.5,ssim_must_not_decrease=True,psnr_pass=deltas['psnr']['mean']>=.5,ssim_pass=aggregates['T022C']['ssim']['mean']>=aggregates['T022A']['ssim']['mean']),
    ...
    verification=dict(passed=True,all100_gates_identical_to_A=True,all100_only_dark_upper_changed=True,
