...
extra=()
if [ "$stage" = B ]; then
    manifest=research_log/T010_manifest.json
    extra=(--calibration research_log/T010_calibration.json --calibration-commit "$3")
fi
python -m ttie.residual_pilot --stage "$stage" --source-sha "$source_sha" \
  --manifest "$manifest" --images /home/wenchang/asdasdsad/wjq/TTIE/shared/t008/val2017 \
