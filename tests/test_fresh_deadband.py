...
from ttie.fresh_deadband.select import cross_vectors
from ttie.fresh_deadband.prepare import freeze_manifest
from ttie.fresh_deadband.evaluate import summarize, evaluate
from ttie.fresh_deadband.common import read, sha, write, CONDITIONS, verify_prepared, preparation_bindings
...
class FreshDirectionTests(unittest.TestCase):
    def test_prepared_mapping_binding_rejects_relabel_and_hash_refresh(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            write(root/'manifest_frozen.json',dict(manifest_sha256=sha(root/'manifest.json'),exclusions_sha256=sha(Path('research_log/T019D_exclusions.json'))))
    ...
    def test_manifest_is_first40_after_exclusions_and_original_eligibility(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            def fake_read(p):return prior if str(p).endswith('T019D_exclusions.json') else original_read(p)
            with patch('ttie.fresh_deadband.prepare.preflight',return_value=({'selector_receipt_sha256':'pin'},{})),patch('ttie.fresh_deadband.prepare.read',side_effect=fake_read):
...
import tempfile
from ttie.fresh_deadband.select import install_reference_barrier
with tempfile.TemporaryDirectory() as d:
...
"""
    ...
    def test_evaluation_cannot_open_reference_before_global_freeze(self):
        ...
        with patch('ttie.fresh_deadband.evaluate.preflight',return_value=({},{})),patch('ttie.fresh_deadband.evaluate.read',side_effect=denied):
    ...
    def test_literal_five_gates_and_harmful_cases(self):
        rows=[dict(row_index=i,condition=c,H0=1.,H1=h,H_star=.94,movement='both') for i,(c,h) in enumerate(zip(CONDITIONS,(.96,.96,.94)))]
    ...
    def test_evaluation_requires_independent_replay_before_reference_load(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            with patch('ttie.fresh_deadband.evaluate.preflight',return_value=({},{})),patch('ttie.fresh_deadband.evaluate.load_image') as loader:
