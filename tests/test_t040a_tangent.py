import torch
from research_log.T040A_tangent.core import probe_raw,masks,classify
...
def test_joint_deficit_gate():
    ...
    assert result()=='source high-gain tangent deficit supported'
    assert result(gc=.251)=='source high-gain tangent deficit not supported / target-domain state shift remains stronger'
    assert result(gp=.56)=='source high-gain tangent deficit not supported / target-domain state shift remains stronger'
