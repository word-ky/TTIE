...
from ttie import ev_range_ttt,lolv2_ev_core,lolv2_budget_core
from ttie.lolv2_budget_core import low_image_opener,native_rgb,save_episode
...
def test_only_declared_budget_changes():
    ...
    new=inspect.getsource(lolv2_budget_core).replace('T022-D','T022-C').replace('max_steps=80','max_steps=40')
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for mode in ['original','changed','withheld']:
            ...
            with patch('PIL.Image.open',low_image_opener({str(low.resolve())},opened)):
                ...
                result,t,decision=ev_range_ttt.trajectory(image,scorer(),RECEIPT,head,max_steps=80)
...
def test_metric_arithmetic_unchanged():
    base=Path('scripts/evaluate_t022a.py').read_text()
    new=Path('scripts/evaluate_t022d.py').read_text().replace("(80 if any(d['gate']['active']) else 0)","(40 if any(d['gate']['active']) else 0)")
    assert ast.dump(ast.parse(base))==ast.dump(ast.parse(new))


def test_final_saturation_separates_active_inactive_and_collapsed():
    import numpy as np
    from scripts.compare_t022d import accumulate_final,finish_counts
    lower=np.zeros((1,2,2,2));upper=np.ones_like(lower);upper[:,:,1,0]=0
    grid=lower.copy();grid[:,:,0,0]=1
    active=np.array([[True,True],[False,True]])
    counts={};accumulate_final(counts,grid,lower,upper,active);out=finish_counts(counts)
    assert out['ev_active']['coordinates']==3 and out['ev_active']['upper']==1
    assert out['ev_inactive']['coordinates']==1 and out['ev_inactive']['either']==1
    assert out['ev_collapsed']['coordinates']==1
    assert out['gamma_active_noncollapsed']['coordinates']==3
