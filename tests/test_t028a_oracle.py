...
spec=importlib.util.spec_from_file_location('oracle',Path(__file__).parents[1]/'research_log/T028A_oracle/core.py')
...
def test_two_start_step_zero_retention_and_projection():
    ...
    obj=o.SimpleNamespace(active=active,winner=winner);box=o.Gamma05Box(obj,2)
    ...
    assert result['raw_history'].shape==(4,1,2,2,2)
    assert torch.equal(result['raw_history'][result['best_step']],result['best_raw'])
