...
HERE=Path('research_log/T069A');MODEL=Path('research_log/T066A/evidence/model.json')
...
def endpoint_scores(transfer):
    ...
    for rec,d,end in zip(recs,data,ends):
        ...
        row=dict(**score(trace['states'].numpy(),trace['pre_box'].numpy(),fs,kl),**identity(rec,trace,d,fs,kl,transfer))
        row.update(previous_state_hash=thash(trace['states'][kl-1]) if kl else None,pre_box_hash=thash(trace['pre_box'][kl-1]) if kl else None,lower_hash=thash(trace['lower']),upper_hash=thash(trace['upper']))
        rows.append(row)
    assert len(rows)==100
    return rows

def add_ranks(rows):
    return [dict(r,descending_rank=1+sum(x['R_proj']>r['R_proj'] for x in rows),empirical_percentile=sum(x['R_proj']<=r['R_proj'] for x in rows)/len(rows)) for r in rows]

def main(out):
    torch.set_num_threads(1);torch.backends.cuda.matmul.allow_tf32=False;torch.backends.cudnn.allow_tf32=False
    binding=json.loads((HERE/'binding.json').read_bytes())
    for p,h in binding.items():assert sha(p)==h,p
    out.mkdir(parents=True,exist_ok=False);start=time.perf_counter();write(out/'config.json',dict(task='T069-A',source_commit=os.environ['TTIE_SOURCE_COMMIT'],source_binding=binding,started_utc=utc(),optimizer_runs=0,model_fits=0))
    dev=endpoint_scores(False);t99=threshold(dev);write(out/'development_freeze.json',dict(rows=add_ranks(dev),T99_proj=t99,percentile='nearest rank sorted_R[98]',reference_reads=0,frozen_utc=utc()))
    transfer=endpoint_scores(True)
    for r in transfer:r.update(T99_proj=t99,above_T99_proj=r['R_proj']>t99)
    write(out/'transfer_freeze.json',dict(rows=add_ranks(transfer),T99_proj=t99,development_freeze_sha256=sha(out/'development_freeze.json'),reference_reads=0,frozen_utc=utc(),source_commit=os.environ['TTIE_SOURCE_COMMIT']))
    ...
    result=dict(**diagnose(joined),T99_proj=t99,development_above_T99_proj=sum(r['R_proj']>t99 for r in dev),unsafe_rows=[r for r in joined if r['unsafe']],optimizer_runs=0,model_fits=0,completed_utc=utc(),seconds=time.perf_counter()-start)
