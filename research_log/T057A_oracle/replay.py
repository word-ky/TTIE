...
zero_errors=[];errors=[];render_errors=[];field_errors=[];basis_errors=[];steps=[];fields={k:[] for k in ['selected_controls','selected_field']};values={k:[] for k in ['psnr','ssim','t055_psnr','t055_ssim','delta_psnr','delta_ssim']}
for i,(row,item,old,main) in enumerate(zip(f['rows'],split,baseline,pairs)):
 ...
 hist=torch.load(root/'history.pt',weights_only=True,map_location='cpu');v=torch.load(root/'output.pt',weights_only=True,map_location='cpu');initial=torch.load(prior_root/'output.pt',weights_only=True,map_location='cpu');ws=hist['w_c']
 ...
 step=min(range(501),key=lambda j:float(hist['mse'][j]));steps.append(step);assert step==row['best_step']==main['best_step'] and torch.equal(v['w_c'],ws[step])
 ...
 assert torch.equal(v['one'],initial['image']) and thash(v['d_chroma'])==pre['rows'][i]['d_chroma_sha256']
 ...
 y0=v['y0'].numpy();h,w=y0.shape[-2:];k5=np.array([1,4,6,4,1],dtype=np.float64)/16
 def blur(y,k):return convolve1d(convolve1d(y.astype(np.float64),k,axis=-1,mode='mirror'),k,axis=-2,mode='mirror')
 detail=y0.astype(np.float64)-blur(y0,k5);basis=detail-detail.mean(axis=1,keepdims=True);basis_errors.append(float(np.max(np.abs(basis-v['d_chroma'].numpy()))));assert basis_errors[-1]<=1e-6
 zero_sum=float(v['d_chroma'].sum(dim=1).abs().max());assert zero_sum<=1e-6;zero_errors.append(zero_sum)
 expected_c=np.tanh(interpolate(v['w_c'].numpy()[0,0],h,w));field_errors.append(float(np.max(np.abs(expected_c-v['c_c'].numpy()[0,0]))));assert field_errors[-1]<=1e-6
 assert np.max(np.abs(np.tanh(v['w_c'].numpy())-v['c_c_controls'].numpy()))<=1e-6 and v['c_c'].min()>=-1 and v['c_c'].max()<=1
 ...
 expected=np.where(mask,np.clip(v['one'].numpy()+v['c_c'].numpy()*v['d_chroma'].numpy(),0,1),v['one'].numpy());render_errors.append(float(np.max(np.abs(expected-v['image'].numpy()))));assert render_errors[-1]<=1e-6
 assert torch.equal(v['image'][:,:,~torch.from_numpy(mask)],v['one'][:,:,~torch.from_numpy(mask)])
 fields['selected_controls'].extend(v['c_c_controls'].numpy().flatten().tolist());fields['selected_field'].extend(v['c_c'].numpy().flatten().tolist())
...
for name,data in fields.items():
 fv=summary['c_c_distributions'][name];arr=np.asarray(data,dtype=np.float64);assert fv['count']==len(data) and fv['exact_hits']=={str(bound):int(np.count_nonzero(arr==float(np.float32(bound)))) for bound in [-1.,1.]}
