"""Frozen T057 outputs versus accepted T055, reference-only evaluation."""
from core import *
...
for i,(item,row,baseline) in enumerate(zip(split,f['rows'],old)):
    ...
    assert hist['w_c'].shape==(501,1,1,8,8) and hist['mse'].shape==(501,) and torch.isfinite(hist['w_c']).all() and torch.count_nonzero(hist['w_c'][0])==0
    step=int(hist['mse'].argmin());assert step==row['best_step'] and torch.equal(v['w_c'],hist['w_c'][step])
    for name in OLD:assert torch.equal(v[name],oldstate[name]),name
    for name,h in row['hashes'].items():assert thash(v[name])==h
    assert torch.equal(v['one'],oldstate['image']) and thash(v['d_chroma'])==pre['rows'][i]['d_chroma_sha256']
    ...
    fields['selected_controls'].extend(v['c_c_controls'].flatten().tolist());fields['selected_field'].extend(v['c_c'].flatten().tolist())
