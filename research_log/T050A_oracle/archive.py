...
base=Path('/home/wenchang/asdasdsad/wjq/TTIE');run=base/'runs/20260916-004508-ttie-t050a-oracle';pre=base/'runs/20260916-004324-ttie-t050a-preflight';ev=base/'runs'/a.evaluation_run;out=base/'shared/t050a';out.mkdir(exist_ok=True);backup=Path('/media/wenchang/F/wjq/TTIE/shared/t050a');backup.mkdir(exist_ok=True)
...
config=json.loads((audit/'config.json').read_bytes());release=base/'releases/20260916-004244-ttie-t050a-extension'
for name,h in config['source_binding'].items():assert sha(release/name)==h,name
full=backup/'T050A_full.tar'
with tarfile.open(full,'w') as t:
    t.add(run,arcname='oracle_run');t.add(pre,arcname='preflight_run');t.add(ev,arcname='evaluation_run');t.add(base/'runs/20260916-023442-ttie-t050a-replay',arcname='replay_run')
compact=out/'T050A_compact.tar.gz'
with tarfile.open(compact,'w:gz') as t:
    ...
    for path,name in [(run,'oracle_run'),(pre,'preflight_run'),(ev,'evaluation_run'),(base/'runs/20260916-023442-ttie-t050a-replay','replay_run')]:
...
(out/'T050A_archives.json').write_text(json.dumps(r,indent=2)+'\n');print(json.dumps(r))
