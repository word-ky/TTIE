"""REFERENCE_ORACLE_ONLY: tone only; all T048 coordinates frozen, exactly1000updates."""
from core import *
...
write(a.out/'config.json',dict(label=LABEL,created_utc=utc(),preflight_sha256=sha(a.preflight/'preflight.json'),preflight_completed_utc=pre['completed_utc'],split_sha256=SPLIT_SHA,optimizer='fresh Adam at exact T049 selected q; fresh moments',lr=.03,segments=8,knots_x=[i/8 for i in range(9)],parameterization='normalized softplus increments, q initialized at T049 selected state, endpoints0/1',updates_per_image=1000,starts_per_image=1,objective='full RGB MSE float64 accumulation through frozen float32 renderer plus lift',lift_bounds=[-.2,.2],all_t048_coordinates_frozen=True,insertion='common gain -> additive lift -> accepted identity contrast arithmetic -> clamp -> monotonic tone LUT -> hard gate compositing',selection='earliest strict minimum over steps0..1000',gpu=torch.cuda.get_device_name(),torch=torch.__version__,cuda=torch.version.cuda,source_binding=pre['source_binding']))
rows=[]
for i,(r,b) in enumerate(zip(split,pre['rows'])):
    ...
    low=native(a.low_root/r['low']).cuda();assert thash(lifts[i])==b['lift_sha256'];model=Tone(low,starts[i].cuda(),b['gate'],b['box'],lifts[i].cuda())
    assert thash(qs[i])==b['q_sha256']
    with torch.no_grad():model.q.copy_(qs[i].cuda())
    reference=native(a.normal_root/r['normal']).cuda();result=optimize(model,reference,steps=1000)
    assert result['updates']==1000 and torch.equal(result['q'][0],qs[i]) and torch.count_nonzero(result['q'][:,~model.active.flatten().cpu()])==0
    assert torch.equal(model.raw.cpu(),starts[i]) and torch.equal(model.lift.cpu(),lifts[i])
    with torch.no_grad():output=model().cpu();selected_knots=knots(model.q).cpu()
    ...
    dest=a.out/f'{i:03d}';dest.mkdir();torch.save(dict(q=result['q'],mse=result['mse']),dest/'history.pt');torch.save(dict(label=LABEL,image=output,raw=model.raw.cpu(),lift=model.lift.cpu(),q=model.q.detach().cpu(),knots=selected_knots),dest/'output.pt')
    step=result['best_step'];row=dict(index=i,low=r['low'],best_step=step,initial_mse=float(result['mse'][0]),best_mse=float(result['mse'][step]),final_mse=float(result['mse'][-1]),updates=1000,seconds=time.perf_counter()-begin,raw_sha256=thash(model.raw),lift_sha256=thash(model.lift),q_sha256=thash(model.q),knots_sha256=thash(selected_knots),output_sha256=thash(output),files={n:sha(dest/n) for n in ['history.pt','output.pt']})
