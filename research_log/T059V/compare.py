...
auth=json.loads(Path('research_log/T059V/authorization.json').read_bytes())
...
for r in records:
    ...
    target=dict(x=field['x'][pos],J=field['J'][pos],q=field['q'][pos],g=field['g'][pos],v1=cache['v1'],y1=cache['y1']);row={k:errors(online[k].numpy(),v.numpy()) for k,v in target.items()};row.update({k:r[k] for k in ['index','image_id','bank_index','state_index']});row['passed']=passed(row);rows.append(row)
...
result=dict(classification='online target-free Jacobian bridge is reproducible on source anchors' if all(r['passed'] for r in rows) else 'online target-free Jacobian bridge is not yet reproducible',passed=sum(r['passed'] for r in rows),total=16,failed_banks=[r['bank_index'] for r in rows if not r['passed']],online_freeze_utc=f['utc'],first_cached_tensor_read_utc=stamp,cached_comparison_inputs=cache_files,counters={k:0 for k in ['clean_reference_reads','reference_gradient_reads','target_domain_access','lolv2_access','official_test_access','inference_reference_leakage']},completed_utc=utc())
