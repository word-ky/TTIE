...
prior_root=Path('/home/wenchang/asdasdsad/wjq/TTIE/runs/20260917-062907-ttie-t058y-shadow')
prior_files=json.loads(Path('research_log/T058Y_archives.json').read_bytes())['files']
...
for index in execution_order:
    ...
    try:
        ...
        if not all(c['passed'] for c in row['criteria'].values()):error='SHADOW_PRIMAL_OR_REVERSE_CRITERION_FAILURE';row['status']='criterion_failed'
            ...
            row['ladder']=[]
            for h in STEPS:
                with torch.no_grad():plus=float(shadow_energy(model64,obj64,head64,h*direction64)[0]);minus=float(shadow_energy(model64,obj64,head64,-h*direction64)[0])
