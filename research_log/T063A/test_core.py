...
from research_log.T063A.reconstruct import render
from research_log.T063A.analyze import oracle
...
def test_saved_states_render_exactly():
 ...
 with torch.no_grad():model.raw.copy_(states[27]);expected=model(x)
 assert torch.equal(images[27],expected) and torch.equal(images[0],x)
