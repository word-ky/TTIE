"""REFERENCE_ORACLE_ONLY: frozen T052 plus fixed local detail; only v changes, exactly500updates."""
from core import *
...
write(a.out/'config.json',dict(label=LABEL,created_utc=utc(),preflight_sha256=sha(a.preflight/'preflight.json'),split_sha256=SPLIT_SHA,optimizer='fresh Adam; only v',lr_v=.05,coefficient_range=[-1,1],blur='separable ordered5tap [1,4,6,4,1]/16 horizontal then vertical; reflect padding, stride1',grid=[8,8],ev_range=[-2,2],interpolation='raw v bilinear align_corners=False, then tanh',updates_per_image=500,starts_per_image=1,objective='full RGB MSE float64 through float32 renderer',all_old_coordinates_frozen=True,insertion='frozen T052 y0 -> clamp(y0+tanh(bilinear(v))*(y0-binomial5x5(y0))); inactive exactly y0',selection='earliest strict minimum steps0..500',gpu=torch.cuda.get_device_name(),torch=torch.__version__,cuda=torch.version.cuda,source_binding=pre['source_binding']))
rows=[]
for i,(r,b) in enumerate(zip(split,pre['rows'])):
    ...
    assert result['updates']==500 and torch.equal(model.u.cpu(),us[i]) and torch.equal(model.b.cpu(),bs[i]) and torch.count_nonzero(result['v'][0])==0 and torch.equal(model.q.cpu(),qs[i])
    ...
    dest=a.out/f'{i:03d}';dest.mkdir();torch.save(dict(v=result['v'],mse=result['mse']),dest/'history.pt');torch.save(dict(label=LABEL,image=output,raw=model.raw.cpu(),lift=model.lift.cpu(),q=model.q.detach().cpu(),knots=selected_knots,u=model.u.detach().cpu(),b=model.b.detach().cpu(),ev=ev,v=model.v.detach().cpu(),c=coefficient,c_controls=controls,y0=model.y0.cpu(),detail=model.detail.cpu()),dest/'output.pt')
    step=result['best_step'];row=dict(index=i,low=r['low'],best_step=step,initial_mse=float(result['mse'][0]),best_mse=float(result['mse'][step]),final_mse=float(result['mse'][-1]),updates=500,seconds=time.perf_counter()-begin,raw_sha256=thash(model.raw),lift_sha256=thash(model.lift),q_sha256=thash(model.q),u_sha256=thash(model.u),b_sha256=thash(model.b),v_sha256=thash(model.v),c_sha256=thash(coefficient),c_controls_sha256=thash(controls),y0_sha256=thash(model.y0),detail_sha256=thash(model.detail),ev_sha256=thash(ev),knots_sha256=thash(selected_knots),output_sha256=thash(output),files={n:sha(dest/n) for n in ['history.pt','output.pt']})
