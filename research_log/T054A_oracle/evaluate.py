"""Frozen T054 outputs versus accepted T052, reference-only evaluation."""
from core import *
...
for i,(item,row,baseline) in enumerate(zip(split,f['rows'],old)):
    ...
    assert hist['v'].shape==(501,1,1,8,8) and hist['mse'].shape==(501,) and torch.isfinite(hist['v']).all() and torch.count_nonzero(hist['v'][0])==0
    ...
    assert thash(v['image'])==row['output_sha256'] and thash(v['b'])==row['b_sha256'] and thash(v['v'])==row['v_sha256'] and torch.equal(v['y0'],start['image'])
...
def distribution(v):
    values=np.asarray(v,dtype=np.float64);d=dict(count=len(v),min=float(values.min()),max=float(values.max()),**stats(values));d['exact_hits']={str(bound):int(np.count_nonzero(values==float(np.float32(bound)))) for bound in [-1.,1.]};return d
