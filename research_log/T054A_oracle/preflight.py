"""Freeze all100 low-only T052 identities before any normal decode."""
from core import *
...
assert sha(a.accepted/'config.json')=='f3e4a3b7e3c0b9225b1fa2a673c6b1088b4a2fd3cfd03827f1957d2a60408522'
...
o.Image.open=low_only;a.out.mkdir(parents=True,exist_ok=False);rows=[];starts=[];lifts=[];qs=[];us=[];bs=[]
for i,(r,old,b) in enumerate(zip(split,f['rows'],prior['rows'])):
    ...
    assert step==old['best_step'] and torch.equal(saved['u'],hist['u'][step]) and thash(saved['u'])==old['u_sha256'] and torch.equal(saved['b'],hist['b'][step]) and thash(saved['b'])==old['b_sha256'] and thash(saved['q'])==old['q_sha256'] and thash(saved['raw'])==b['raw_sha256']==old['raw_sha256'] and thash(saved['lift'])==b['lift_sha256']==old['lift_sha256'] and thash(saved['image'])==old['output_sha256']
    assert sha(a.low_root/r['low'])==r['low_sha256'];low=native(a.low_root/r['low']).cuda();model=Detail(low,saved['raw'].cuda(),b['gate'],b['box'],saved['lift'].cuda(),saved['q'].cuda(),saved['u'].cuda(),saved['b'].cuda())
    with torch.no_grad():output=model().cpu()
    error=float((output-saved['image']).abs().max());assert error<=1e-6
    rows.append(dict(index=i,low=r['low'],gate=b['gate'],box=b['box'],prior_files=old['files'],prior_best_step=step,raw_sha256=thash(saved['raw']),lift_sha256=thash(saved['lift']),q_sha256=thash(saved['q']),u_sha256=thash(saved['u']),b_sha256=thash(saved['b']),basis_sha256=thash(model.detail),y0_sha256=thash(model.y0),accepted_output_sha256=thash(saved['image']),reconstructed_output_sha256=thash(output),max_abs_error=error,bit_exact=torch.equal(output,saved['image'])))
    starts.append(saved['raw'].clone());lifts.append(saved['lift'].clone());qs.append(saved['q'].clone());us.append(saved['u'].clone());bs.append(saved['b'].clone())
assert len(opened)==100
torch.save(torch.stack(bs),a.out/'bs.pt');torch.save(torch.stack(us),a.out/'us.pt');torch.save(torch.stack(starts),a.out/'starts.pt');torch.save(torch.stack(lifts),a.out/'lifts.pt');torch.save(torch.stack(qs),a.out/'qs.pt');write(a.out/'preflight.json',dict(label=LABEL,completed_utc=utc(),rows=rows,normal_decodes=0,opened_lows=opened,starts_sha256=sha(a.out/'starts.pt'),lifts_sha256=sha(a.out/'lifts.pt'),qs_sha256=sha(a.out/'qs.pt'),us_sha256=sha(a.out/'us.pt'),bs_sha256=sha(a.out/'bs.pt'),prior_freeze_sha256=PRIOR_FREEZE,prior_preflight_sha256=PRIOR_PREFLIGHT,split_sha256=SPLIT_SHA,source_binding=binding,source_binding_sha256=sha(a.binding),all100_max_abs=max(r['max_abs_error'] for r in rows),all100_bit_exact=all(r['bit_exact'] for r in rows),seconds=time.perf_counter()-start))
