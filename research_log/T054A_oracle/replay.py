...
errors=[];render_errors=[];field_errors=[];basis_errors=[];steps=[];fields={k:[] for k in ['selected_controls','selected_field']};values={k:[] for k in ['psnr','ssim','t052_psnr','t052_ssim','delta_psnr','delta_ssim']}
for i,(row,item,old,main) in enumerate(zip(f['rows'],split,baseline,pairs)):
 ...
 assert vs.shape==(501,1,1,8,8) and hist['mse'].shape==(501,) and torch.isfinite(vs).all() and torch.isfinite(hist['mse']).all() and torch.count_nonzero(vs[0])==0
 step=min(range(501),key=lambda j:float(hist['mse'][j]));steps.append(step);assert step==row['best_step']==main['best_step'] and torch.equal(v['v'],vs[step])
 ...
 assert torch.equal(v['y0'],initial['image']) and thash(v['y0'])==pre['rows'][i]['y0_sha256'] and thash(v['detail'])==pre['rows'][i]['basis_sha256']
...
classification='local-detail capacity supported' if statistics.mean(values['delta_psnr'])>=.5 and statistics.median(values['delta_psnr'])>=.25 and statistics.mean(values['delta_ssim'])>=.020 else 'not supported under fixed local-detail probe'
