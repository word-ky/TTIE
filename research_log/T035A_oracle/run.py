...
assert pre['all100_shared_gain_regression'] and pre['shared_renderer_max_abs']<=1e-6
...
config=dict(label=LABEL,task='T035-A',seed=7,optimizer='Adam',lr=.05,updates_per_start=500,
    ...
    bounds='exact T026A EVgamma plus activeRGBsharedgain.5..2 inactiveidentity; contrast1',
    ...
    split_sha256=sha(a.split),code_sha256={str(p):sha(p) for p in Path('research_log/T035A_oracle').glob('*.py')})
