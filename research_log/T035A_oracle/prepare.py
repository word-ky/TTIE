...
for k in ['split','low-root','accepted','prior-oracle','prior-preflight','full-wb','out']:p.add_argument('--'+k,type=Path,required=True)
...
assert o.sha(a.full_wb/'per_image.csv')=='686a1497a6a52b3f3ed51e2fd5f4b6d916c3015aebd7dd95a31879c858db869b'
fullfreeze=json.loads((a.full_wb/'freeze.json').read_bytes())
assert o.sha(a.full_wb/'freeze.json')=='8a1f60005329e75c362c0f37b7ba84c555059c8b813d9ac157ad6fcf2482be04'
for r in fullfreeze['rows']:
    for n,h in r['files'].items():assert o.sha(a.full_wb/f'{r["index"]:03d}'/n)==h
...
for i,r in enumerate(split):
    ...
    assert error<=1e-6
    shared_errors=[]
    full,_=wb.frozen_model(decision,low)
    for gain in [torch.tensor([-.8,.2,.7,-.3]).reshape(1,1,2,2),torch.tensor([.9,-.6,.1,.5]).reshape(1,1,2,2)]:
        with torch.no_grad():
            model.raw.copy_(pair[1].cuda());model.raw[:,2:].copy_(gain.cuda());box(model)
            full.raw.copy_(torch.cat([model.raw[:,:2],model.raw[:,2:3].expand(-1,3,-1,-1)],1))
            difference=float((model(low)-full(low)).abs().max())
        assert difference<=1e-6;shared_errors.append(difference)
    rows.append(dict(shared_gain_max_abs=shared_errors,index=i,low=r['low'],files=binding['files'],identity_wb_max_abs=values,accepted_output_max_abs=error,gate=decision['gate'],box=decision['diagnostics']['action_box']))
...
o.write(a.out/'preflight.json',dict(label=o.LABEL,completed_utc=o.utc(),rows=rows,starts_sha256=o.sha(a.out/'bound_starts.pt'),split_sha256=o.sha(a.split),
    ...
    prior_csv_sha256=o.sha(a.prior_oracle/'per_image.csv'),full_wb_freeze_sha256=o.sha(a.full_wb/'freeze.json'),full_wb_csv_sha256=o.sha(a.full_wb/'per_image.csv'),shared_renderer_max_abs=max(max(r['shared_gain_max_abs']) for r in rows),all100_shared_gain_regression=True,all100_identity_wb_regression=True,normal_decodes=0,opened_lows=opened,
