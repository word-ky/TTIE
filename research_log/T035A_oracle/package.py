"""Archive the completed T035 diagnostic; never rerun optimization or metrics."""
from pathlib import Path
import hashlib, json, tarfile, shutil, datetime

base=Path('/home/wenchang/asdasdsad/wjq/TTIE')
backup=Path('/media/wenchang/F/wjq/TTIE/shared/t035a')
run=base/'runs/20260915-023649-ttie-t035a-oracle'
pre=base/'runs/20260915-023543-ttie-t035a-preflight'
out=run/'artifacts/REFERENCE_ORACLE_ONLY'
dest=base/'shared/t035a';dest.mkdir(parents=True,exist_ok=True);backup.mkdir(parents=True,exist_ok=True)
...
(dest/'T035A_archives.json').write_text(json.dumps(receipt,indent=2)+'\n')
shutil.copy2(dest/'T035A_archives.json',backup/'T035A_archives.json')
