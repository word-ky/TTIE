...
assert len(rows)==100 and len(bound)==1200
def quantile(v,q):
...
for r in rows:
    for m in ['psnr','ssim']:assert abs(float(r['wb_minus_common_'+m])-(float(r['full_wb_'+m])-float(r['oracle_'+m])))<1e-12
...
for r in freeze['rows']:
    for name in ['oracle_states.json']:
...
c='common-mode explains most WB gain' if mean>=1.835968277 and median>=1.431879733 else ('chromatic degrees essential' if mean<.5 else 'mixed attribution')
assert c==summary['classification']
receipt=dict(rows=100,retained_artifact_hashes_verified=100,all_summaries_recomputed=True,max_abs_error=max(errors),classification=c)
(a.evidence.parent/'T035A_local_verification.json').write_text(json.dumps(receipt,indent=2)+'\n')
