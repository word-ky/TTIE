"""Freeze all100 low-only T046 identities before any normal decode."""
from core import *
...
for i,(r,old,b) in enumerate(zip(split,f['rows'],prior['rows'])):
    ...
    assert step==old['best_step'] and torch.equal(saved['raw'],hist['raw'][step]) and thash(saved['raw'])==old['raw_sha256'] and thash(saved['image'])==old['output_sha256']
    assert sha(a.low_root/r['low'])==r['low_sha256'];low=native(a.low_root/r['low']).cuda();model=Lift(low,saved['raw'].cuda(),b['gate'],b['box'])
    with torch.no_grad():output=model().cpu()
    error=float((output-saved['image']).abs().max());assert error<=1e-6
    rows.append(dict(index=i,low=r['low'],gate=b['gate'],box=b['box'],prior_files=old['files'],prior_best_step=step,raw_sha256=thash(saved['raw']),accepted_output_sha256=thash(saved['image']),reconstructed_output_sha256=thash(output),max_abs_error=error,bit_exact=torch.equal(output,saved['image'])))
