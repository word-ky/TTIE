"""REFERENCE_ORACLE_ONLY: only bounded physical lift; frozen T046 legacy, exactly500updates."""
from core import *
...
write(a.out/'config.json',dict(label=LABEL,created_utc=utc(),preflight_sha256=sha(a.preflight/'preflight.json'),preflight_completed_utc=pre['completed_utc'],split_sha256=SPLIT_SHA,optimizer='fresh Adam at exact T046 winner; zero lift; only lift optimized',lr=.01,updates_per_image=500,starts_per_image=1,objective='full RGB MSE float64 accumulation through frozen float32 renderer plus lift',lift_bounds=[-.2,.2],legacy_coordinates_frozen=True,insertion='common gain -> additive lift -> accepted identity contrast arithmetic -> clamp -> hard gate compositing',selection='earliest strict minimum over steps0..500',gpu=torch.cuda.get_device_name(),torch=torch.__version__,cuda=torch.version.cuda,source_binding=pre['source_binding']))
rows=[]
for i,(r,b) in enumerate(zip(split,pre['rows'])):
    ...
    low=native(a.low_root/r['low']).cuda();model=Lift(low,starts[i].cuda(),b['gate'],b['box'])
    reference=native(a.normal_root/r['normal']).cuda();result=optimize(model,reference,steps=500)
    assert result['updates']==500 and torch.count_nonzero(result['lift'][0])==0
    assert torch.equal(model.legacy_raw.cpu(),starts[i]) and torch.count_nonzero(result['lift'][:,~model.active.cpu()])==0
    with torch.no_grad():output=model().cpu()
    assert torch.isfinite(output).all() and 0<=output.min()<=output.max()<=1
    dest=a.out/f'{i:03d}';dest.mkdir();torch.save(dict(lift=result['lift'],mse=result['mse']),dest/'history.pt');torch.save(dict(label=LABEL,image=output,raw=model.legacy_raw.cpu(),lift=model.b.detach().cpu()),dest/'output.pt')
    step=result['best_step'];row=dict(index=i,low=r['low'],best_step=step,initial_mse=float(result['mse'][0]),best_mse=float(result['mse'][step]),final_mse=float(result['mse'][-1]),updates=500,seconds=time.perf_counter()-begin,raw_sha256=thash(model.legacy_raw),lift_sha256=thash(model.b),output_sha256=thash(output),files={n:sha(dest/n) for n in ['history.pt','output.pt']})
