...
assert sha(a.accepted/'freeze.json')==pre['prior_freeze_sha256'] and sha(a.accepted/'pairs.json')==receipt['prior_csv_sha256']=='93e6b6dc1d463d1b088cb30fed5b995797c9f1d7dcb72d2544ac2751b7ef2652'
...
errors=[];values={k:[] for k in ['psnr','ssim','t046_psnr','t046_ssim','delta_psnr','delta_ssim']};steps=[];lifts=[];render_errors=[]
for i,(row,item,old,main) in enumerate(zip(f['rows'],split,baseline,pairs)):
    ...
    assert step==row['best_step']==main['best_step'] and torch.equal(v['lift'],h['lift'][step]) and torch.equal(starts[i],v['raw']) and torch.count_nonzero(h['lift'][0])==0
    ...
    val=dict(psnr=psnr,ssim=ssim,t046_psnr=float(old['psnr']),t046_ssim=float(old['ssim']),delta_psnr=psnr-float(old['psnr']),delta_ssim=ssim-float(old['ssim']))
...
classification='additive-lift marginal capacity supported' if statistics.mean(values['delta_psnr'])>=.5 and statistics.median(values['delta_psnr'])>=.25 else 'additive-lift material marginal capacity not supported under fixed probe'
...
result=dict(status='PASS',images=100,history_states=50100,selected_output_metrics=200,scalar_checks=len(errors),max_abs_error=max(errors),classification=classification,independent_renderer_max_abs=max(render_errors),legacy_raws_exact=True,all_states_finite_bounded=True,all_earliest_selected_raws_exact=True,preflight_before_normals=True,freeze_before_metrics=True,seconds=time.perf_counter()-start)
