...
base=Path('/home/wenchang/asdasdsad/wjq/TTIE');run=base/'runs/20260915-191837-ttie-t047a-oracle';pre=base/'runs/20260915-191729-ttie-t047a-preflight';ev=base/'runs'/a.evaluation_run;out=base/'shared/t047a';out.mkdir(exist_ok=True);backup=Path('/media/wenchang/F/wjq/TTIE/shared/t047a');backup.mkdir(exist_ok=True)
...
config=json.loads((audit/'config.json').read_bytes());release=base/'releases/20260915-191702-ttie-t047a-lift'
for name,h in config['source_binding'].items():assert sha(release/name)==h,name
full=backup/'T047A_full.tar'
...
compact=out/'T047A_compact.tar.gz'
...
(out/'T047A_archives.json').write_text(json.dumps(r,indent=2)+'\n');print(json.dumps(r))
