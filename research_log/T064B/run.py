...
from research_log.T064B.core import perturb,sensitivity,base_choose,choose,calibrate,summarize,SEED,DELTA,RHO
...
class ReadScope:
    ...
    def hook(self, event, args):
        ...
        raise PermissionError('T064-B target-free read boundary: '+str(p))
...
def render_statistics(low,trace):
    x=low.cuda();model=CommonRegion2(trace['active']).to(x).eval().requires_grad_(False)
    plus,minus=perturb(x);images=[];parts=[];totals=[];values=[];hashes=[]
    with torch.no_grad():
        for state in trace['states'][:28]:
            model.raw.copy_(state.to(x));y=model(x);components=losses(x,y)
            yp=model(plus);ym=model(minus)
            parts.append(components.cpu().numpy().astype(np.float64).tolist())
            totals.append(float(components@components.new_tensor([1.,10.,5.])))
            values.append(sensitivity(yp,ym,plus,minus))
            hashes.append([thash(yp.cpu()),thash(ym.cpu())]);images.append(y.cpu().clone())
    np.testing.assert_allclose(totals,trace['values'][:28],rtol=0,atol=1e-7)
    return images,parts,totals,values,hashes,[thash(plus.cpu()),thash(minus.cpu())]


def reconstruct(out, transfer=False, tau=None):
    ...
    with scope:
        for i, (item, rec) in enumerate(zip(items, recs)):
            ...
            low, trace = state_input(raw, rec, transfer); images,parts,totals,values,pair_hashes,perturbed_hashes=render_statistics(low,trace)
            ...
            row = dict(index=i, low=item['low'], components=parts,totals=totals,base_step=base_step,sensitivity=values,pair_hashes=pair_hashes,perturbed_input_hashes=perturbed_hashes,hashes=hashes)
            if transfer:
                k = choose(values,base_step,tau); row.update(selected_step=k, output_hash=hashes[k])
...
def main(out):
    ...
    write(out/'config.json', dict(task='T064-B', source_commit=os.environ['TTIE_SOURCE_COMMIT'], source_binding=binding,
    ...
    table, best, passed = calibrate(dev,quality)
    write(out/'threshold_candidates.json', table)
    with (out/'threshold_candidates.csv').open('x', newline='') as f:
        fields=['tau','mean_psnr','mean_delta_psnr','median_delta_psnr','regressions_t026','worst_delta_t026','mean_delta_ssim','eligible']
        writer=csv.DictWriter(f,fieldnames=fields,extrasaction='ignore');writer.writeheader();writer.writerows(table)
    rule = dict(rule='largest k in0..k_rho with S_k<=tau; else0',statistic='mean_abs(f(x_plus)-f(x_minus))/max(mean_abs(x_plus-x_minus),1e-8)',seed=SEED,delta=DELTA,rho=RHO,perturbation='CPU torch.Generator manual_seed64064; int64 randint0,2 regenerated per shape; e=2*r-1; clip x+-e/255',
                arithmetic='float32 GPU renderer and mean reductions; binary64 numerator/denominator division',candidate_definition='all unique finite development S_k with k<=k_rho; no sentinels',
                tau=best['tau'] if best else None, calibration=best, calibration_pass=passed, candidate_count=len(table),
    ...
    rule['development_selected_steps']=[choose(r['sensitivity'],r['base_step'],rule['tau']) for r in dev] if passed else []
    rule['development_base_steps']=[r['base_step'] for r in dev]
    ...
    rows, reads = reconstruct(out, True, rule['tau'])
    ...
    result=dict(classification='TARGET_FREE_STABILITY_GUARD_TRANSFER_PASS' if all(metrics['gates'].values()) else 'TRANSFER_NEGATIVE',
        label='exposed-cohort transfer audit; not fresh qualification', tau=rule['tau'], **metrics,
