...
from research_log.T064A.analyze import score
import torch
...
def main(out):
 ...
 assert sha(MANIFEST)==COHORT and sha(RAW/'freeze.json')==FREEZE and f['outputs']==2800 and f['selected_exact']==100
 ...
 for n,h in json.loads((out/'config.json').read_bytes())['source_binding'].items():assert sha(n)==h,n
 allowed={str((LOWROOT/r['low']).resolve()) for r in f['rows']}|{str((RAW/f"{r['index']:03d}"/'T063_trace.pt').resolve()) for r in f['rows']}
 assert set(f['reads'])==allowed
 torch.set_num_threads(1);torch.backends.cuda.matmul.allow_tf32=False;torch.backends.cudnn.allow_tf32=False
 for rec in f['rows']:
  directory=RAW/f"{rec['index']:03d}";assert sha(directory/'T063_trace.pt')==rec['input_trace_sha256']
  trace=torch.load(directory/'T063_trace.pt',weights_only=True,map_location='cpu');low=native_rgb(LOWROOT/rec['low']).cuda();model=CommonRegion2(trace['active']).to(low).requires_grad_(False)
  with torch.no_grad():
   for k in range(28):
    model.raw.copy_(trace['states'][k].to(low));actual=model(low).cpu()
    assert thash(actual)==rec['rendered_hashes'][k]
    if k==rec['selected_step']:assert thash(actual)==rec['accepted_selected_hash']
  del trace,low,model
 refs=json.loads(Path('research_log/T063D/reference_inputs.json').read_bytes());assert sha(RAW/'paired.json')==PAIRED;prior=json.loads((RAW/'paired.json').read_bytes())
 ...
 classification='FRESH_TAIL_SELECTION_LIMITED' if all_reachable and all(gates.values()) else 'FRESH_TAIL_TRAJECTORY_LIMITED';assert classification==s['classification']
 write(out/'verification.json',dict(status='PASS',classification=classification,output_hashes=2800,selected_exact=100,max_metric_error=max(errors),gates=gates,verified_utc=utc()));print(classification,'VERIFIED',max(errors),flush=True)
