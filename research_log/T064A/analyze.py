import argparse,json,zipfile,io,time,math,statistics,csv
...
def score(job):
 ...
 old=RAW/f"{rec['index']:03d}";assert sha(old/'outputs.pt')==rec['input_outputs_sha256'];saved=torch.load(old/'outputs.pt',weights_only=True,map_location='cpu');assert torch.equal(images[rec['selected_step']],saved['T063'])
 stamp=utc()
 ...
 for key in ['psnr','ssim']:assert abs(metrics[rec['selected_step']][key]-prior['metrics']['T063'][key])<1e-10
 return dict(index=rec['index'],selected_step=rec['selected_step'],low=item['low'],normal=item['normal'],reference_read_utc=stamp,normal_sha256=reference['normal_sha256'],states=metrics,controls=controls)

def main(out):
 start=time.perf_counter();freeze=json.loads((out/'freeze.json').read_bytes());assert freeze['outputs']==2800 and freeze['selected_exact']==100 and sha(MANIFEST)==COHORT
 ...
 rows=json.loads(MANIFEST.read_bytes())['selected'];refs=json.loads(Path('research_log/T063D/reference_inputs.json').read_bytes());assert sha(RAW/'paired.json')==PAIRED;prior=json.loads((RAW/'paired.json').read_bytes())
 ...
 failures=[r for r in scored if r['states'][r['selected_step']]['psnr']-r['controls']['T026']['psnr']< -5.614]
 result=dict(label='REFERENCE_ORACLE_ONLY',classification='FRESH_TAIL_SELECTION_LIMITED' if not any(r['safety_unreachable'] for r in scored) and all(gates.values()) else 'FRESH_TAIL_TRAJECTORY_LIMITED',reachable_images=sum(not r['safety_unreachable'] for r in scored),unreachable_indices=[r['index'] for r in scored if r['safety_unreachable']],gates=gates,mean_delta_psnr=float(d.mean()),median_delta_psnr=float(np.median(d)),regressions_t026=int((b<0).sum()),worst_delta_t026=float(b.min()),mean_delta_ssim=float(ds.mean()),mean_psnr=float(np.mean([s['psnr'] for s in selected])),mean_ssim=float(np.mean([s['ssim'] for s in selected])),selected_failure_indices=[r['index'] for r in failures],freeze_sha256=sha(out/'freeze.json'),freeze_utc=freeze['completed_utc'],first_reference_read_utc=first,seconds=time.perf_counter()-start,completed_utc=utc(),optimizer_runs=0)
 assert [r['index'] for r in failures]==[16,86]
 for r in scored:
  ranges=[]
  for k in r['reachable_steps']:
   if ranges and k==ranges[-1][1]+1:ranges[-1][1]=k
   else:ranges.append([k,k])
  r['safe_step_ranges']=ranges
 for filename,records in [('per_step.csv',scored),('tail_trajectories.csv',failures)]:
  with (out/filename).open('x',newline='') as stream:
   writer=csv.writer(stream);writer.writerow(['index','low','step','psnr','ssim','delta_T026','safety_reachable','selected_step','oracle_step'])
   for r in records:
    for k,m in enumerate(r['states']):writer.writerow([r['index'],r['low'],k,m['psnr'],m['ssim'],m['psnr']-r['controls']['T026']['psnr'],k in r['reachable_steps'],r['selected_step'],r['oracle_step']])
 write(out/'per_image.json',scored);write(out/'tail_failures.json',failures);write(out/'result.json',result);print(json.dumps(result),flush=True)
