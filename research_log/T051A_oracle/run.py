"""REFERENCE_ORACLE_ONLY: spatial exposure only; all T050 coordinates frozen, exactly500updates."""
from core import *
...
write(a.out/'config.json',dict(label=LABEL,created_utc=utc(),preflight_sha256=sha(a.preflight/'preflight.json'),split_sha256=SPLIT_SHA,optimizer='fresh Adam; only u',lr=.05,grid=[8,8],ev_range=[-2,2],interpolation='bilinear align_corners=False after2tanh',updates_per_image=500,starts_per_image=1,objective='full RGB MSE float64 through float32 renderer',all_old_coordinates_frozen=True,insertion='affine clamp -> multiply by2**e -> clamp -> frozen tone -> hard gate',selection='earliest strict minimum steps0..500',gpu=torch.cuda.get_device_name(),torch=torch.__version__,cuda=torch.version.cuda,source_binding=pre['source_binding']))
rows=[]
for i,(r,b) in enumerate(zip(split,pre['rows'])):
    ...
    low=native(a.low_root/r['low']).cuda();assert thash(lifts[i])==b['lift_sha256'];model=Spatial(low,starts[i].cuda(),b['gate'],b['box'],lifts[i].cuda(),qs[i].cuda())
    assert thash(qs[i])==b['q_sha256']
    assert torch.equal(model.q.cpu(),qs[i])
    reference=native(a.normal_root/r['normal']).cuda();result=optimize(model,reference,steps=500)
    assert result['updates']==500 and torch.count_nonzero(result['u'][0])==0 and torch.equal(model.q.cpu(),qs[i])
    assert torch.equal(model.raw.cpu(),starts[i]) and torch.equal(model.lift.cpu(),lifts[i])
    with torch.no_grad():output=model().cpu();selected_knots=model.y.cpu();ev=model.ev().cpu()
    ...
    dest=a.out/f'{i:03d}';dest.mkdir();torch.save(dict(u=result['u'],mse=result['mse']),dest/'history.pt');torch.save(dict(label=LABEL,image=output,raw=model.raw.cpu(),lift=model.lift.cpu(),q=model.q.detach().cpu(),knots=selected_knots,u=model.u.detach().cpu(),ev=ev),dest/'output.pt')
    step=result['best_step'];row=dict(index=i,low=r['low'],best_step=step,initial_mse=float(result['mse'][0]),best_mse=float(result['mse'][step]),final_mse=float(result['mse'][-1]),updates=500,seconds=time.perf_counter()-begin,raw_sha256=thash(model.raw),lift_sha256=thash(model.lift),q_sha256=thash(model.q),u_sha256=thash(model.u),ev_sha256=thash(ev),knots_sha256=thash(selected_knots),output_sha256=thash(output),files={n:sha(dest/n) for n in ['history.pt','output.pt']})
