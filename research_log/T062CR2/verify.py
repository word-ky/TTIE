...
def main(out):
    torch.set_num_threads(1);manifest=Path('research_log/T062CR1/manifest.json');assert digest(manifest)=='e8a66a350bcce5282df2e8114355afc434bcd0f4ea69e54957ebb7618f9183c2'
    ...
    with zipfile.ZipFile('/media/wenchang/F/wjq/TTIE/shared/t022a/LOL-v2.zip') as z:
        for i,(r,f,p,o) in enumerate(zip(rows,freeze['rows'],paired,opened)):
            ...
            t=torch.load(directory/'T062_trace.pt',weights_only=True,map_location='cpu');assert len(t['gradients'])==27 and len(t['states'])==28 and t['selected_step']==27 and torch.count_nonzero(t['states'][0])==0
    ...
    d=[r['T062']['psnr']-r['T036']['psnr'] for r in allmetrics];b=[r['T062']['psnr']-r['T026']['psnr'] for r in allmetrics];s=[r['T062']['ssim']-r['T036']['ssim'] for r in allmetrics]
    ...
    for n in ['T026','T036','T062','identity']:
    ...
    assert gates==summary['gates'];verdict='FRESH_QUALIFICATION_PASS' if all(gates.values()) else 'NEGATIVE';assert verdict==summary['verdict']
    v=dict(status='PASS',verdict=verdict,final_outputs=300,metrics_checked=800,max_metric_error=max(errors),gates=gates,cohort_sha256=digest(manifest),verified_utc=datetime.now(timezone.utc).isoformat())
