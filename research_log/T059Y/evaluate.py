...
validate_inputs(json.loads(Path('research_log/T059Y/evaluation_binding.json').read_bytes()))
pairing=json.loads(Path('research_log/T059Y/evaluation_pairing.json').read_bytes());assert pairing['split_sha256']=='b88c8347005984b5523b117b52c0c068672fe172eb7c9aa5b60102d350e2d85b';refs=[];rows=[]
for r,s in zip(records,pairing['selected']):
    ...
    t=torch.load(out/r['file'],weights_only=True,map_location='cpu');base=torch.load(r['base_file'],map_location='cpu',weights_only=True);assert torch.equal(base['image'],t['y0'])
    ...
    for metric in ['mse','psnr','ssim']:row[metric+'_change']=row['ours_'+metric]-row['base_'+metric]
    rows.append(row)
