...
from research_log.T058Y.numerics import STEPS,boundary,base_checks,fd_check
import copy
import traceback
...
prior_root=Path('/home/wenchang/asdasdsad/wjq/TTIE/runs/20260917-054710-ttie-t058x-backend')
prior_files=json.loads(Path('research_log/T058X_archives.json').read_bytes())['files']
for n,h in prior_files.items():assert sha(prior_root/n)==h
rows=[dict(**x,status='not_run',E32=None,d32_rev=None,E64=None,d64_rev=None,criteria=None,ladder=None,boundary=None) for x in selection]
...
error=None;hash_receipts=[];execution_order=[3]+[i for i in range(16) if i!=3]
for index in execution_order:
    row=rows[index];entry=banks[row['bank_index']];directory=a.bank_root/entry['directory']
    ...
    try:
        v64,y64,phi64=energy(model64,obj64,head64)
        ...
        row['criteria']=base_checks(row['E32'],row['d32_rev'],row['E64'],row['d64_rev'])
        if not all(c['passed'] for c in row['criteria'].values()):error='SHADOW_PRIMAL_OR_REVERSE_CRITERION_FAILURE';row['status']='criterion_failed'
        else:
            row['ladder']=[]
            for h in STEPS:
                with torch.no_grad():plus=float(energy(model64,obj64,head64,h*direction64)[0]);minus=float(energy(model64,obj64,head64,-h*direction64)[0])
                assert np.isfinite(plus) and np.isfinite(minus)
                row['ladder'].append(dict(h=h,plus=plus,minus=minus,central=(plus-minus)/(2*h),plus_secant=(plus-row['E64'])/h,minus_secant=(row['E64']-minus)/h))
            row['fd_criterion']=fd_check(row['d64_rev'],row['boundary']['boundary_directional_count'],row['ladder']);row['status']=row['fd_criterion']['classification']
            if row['status']=='unresolved':error='FLOAT64_FD_CRITERION_FAILURE'
