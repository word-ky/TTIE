...
from research_log.T058AB.chain import full_chain,CREDIBLE,UNRESOLVED
import copy
...
prior_root=Path('/home/wenchang/asdasdsad/wjq/TTIE/runs/20260917-082811-ttie-t058aa-clamp')
prior_files=json.loads(Path('research_log/T058AA_archives.json').read_bytes())['files']
...
error=None;hash_receipts=[];execution_order=list(range(16))
for index in execution_order:
    ...
    try:
        ...
        row['full_gradient']=full_chain(model64,obj64,head64,direction64,y64,row['E32'],row['E64'],g,g64)
        row['criteria']=row['full_gradient']['criteria']
        if all(c['passed'] for c in row['criteria'].values()):row['status']='credible'
        else:error='FULL_GRADIENT_CRITERION_FAILURE';row['status']='unresolved'
