...
`test_chunk_attention.py` compared the frozen official attention and the chunked implementation on nine deterministic float32 synthetic cases (31/513/1025 tokens × no/mixed/all-zero masks), including chunk boundaries. Every case passed `atol=1e-6, rtol=1e-5`; maximum absolute difference was 1.1921e-7. No UHD-LL image or reference was used. The test is scoped to the exact attention primitive; a full-model synthetic and one authorized UHD-LL smoke are the next validation steps.
