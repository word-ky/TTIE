...
from research_log.T065A.core import features,fit,predict,base_choose,choose,summarize,FEATURES,RHO,LAMBDA
...
class ReadScope:
    ...
    def hook(self, event, args):
        ...
        raise PermissionError('T065-A target-free read boundary: '+str(p))
    ...
    with scope:
        for i, (item, rec) in enumerate(zip(items, recs)):
            ...
            if transfer:
                ...
                row.update(predictions=predictions,selected_step=k,output_hash=hashes[k])
...
def main(out):
    ...
    write(out/'config.json', dict(task='T065-A', source_commit=os.environ['TTIE_SOURCE_COMMIT'], source_binding=binding,
    ...
    for row,q in zip(dev,quality):
        for k in range(28):training.append(dict(index=row['index'],low=row['low'],step=k,features=row['features'][k],target=q['psnr'][k]-q['t026_psnr']))
    write(out/'training_table.json',training)
    with (out/'training_table.csv').open('x',newline='') as f:
        writer=csv.writer(f);writer.writerow(['index','step',*FEATURES,'target'])
        for row in training:writer.writerow([row['index'],row['step'],*row['features'],row['target']])
    head=fit([r['features'] for r in training],[r['target'] for r in training])
    write(out/'model.json',head)
    dev_predictions=[predict(r['features'],head).tolist() for r in dev]
    steps=[choose(p,r['base_step']) for p,r in zip(dev_predictions,dev)]
    development=dict(predictions=dev_predictions,selected_steps=steps,base_steps=[r['base_step'] for r in dev],metrics=summarize(quality,steps),
        choices_changed=sum(k!=r['base_step'] for k,r in zip(steps,dev)),training_rmse=float(np.sqrt(np.mean((np.asarray(dev_predictions).reshape(-1)-np.asarray([r['target'] for r in training]))**2))))
    write(out/'development_fit.json',development)
    rule=dict(feature_names=FEATURES,feature_definition='exact 11 features in authorization.md; Y=.299R+.587G+.114B; gradient=mean of concatenated absolute valid H and W forward differences; all raw12 coordinates',
        arithmetic='float32 GPU renderer/objective; float64 GPU image/state features and float64 CPU progress/normalization/ridge linear solve; population standard deviations',
        ridge_lambda=LAMBDA,rho=RHO,rule='argmax predicted development-trained paired quality margin within0..k_rho; earliest exact tie',
        model_sha256=sha(out/'model.json'),training_table_sha256=sha(out/'training_table.json'),features_sha256=sha(out/'development_features.json'),development_fit_sha256=sha(out/'development_fit.json'),
    ...
    result=dict(classification='LINEAR_QUALITY_HEAD_TRANSFER_PASS' if all(metrics['gates'].values()) else 'TRANSFER_NEGATIVE',
