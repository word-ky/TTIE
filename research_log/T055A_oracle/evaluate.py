"""Frozen T055 outputs versus accepted T054, reference-only evaluation."""
from core import *
...
for i,(item,row,baseline) in enumerate(zip(split,f['rows'],old)):
    ...
    assert hist['v'].shape==(1001,1,1,8,8) and hist['mse'].shape==(1001,) and torch.isfinite(hist['v']).all() and torch.equal(hist['v'][0],torch.load(a.preflight/'vs.pt',weights_only=True,map_location='cpu')[i])
    step=int(hist['mse'].argmin());assert step==row['best_step'] and torch.equal(v['v'],hist['v'][step])
    for k in ['raw','lift','q','u','b','knots','ev']:assert torch.equal(v[k],start[k]) and thash(v[k])==row[k+'_sha256']
    assert thash(v['image'])==row['output_sha256'] and thash(v['b'])==row['b_sha256'] and thash(v['v'])==row['v_sha256'] and torch.equal(v['y0'],start['y0'])
    assert thash(v['mask'])==pre['rows'][i]['mask_sha256']
    for name in ['c','c_controls','y0','detail']:assert thash(v[name])==row[name+'_sha256']
    ...
    fields['selected_controls'].extend(v['c_controls'].flatten().tolist());fields['selected_field'].extend(v['c'].flatten().tolist())
    rows.append(dict(index=i,low=item['low'],best_step=step,psnr=metrics['psnr'],ssim=metrics['ssim'],t054_psnr=baseline['psnr'],t054_ssim=baseline['ssim'],delta_psnr=metrics['psnr']-baseline['psnr'],delta_ssim=metrics['ssim']-baseline['ssim'],seconds=row['seconds']))
...
s=dict(label=LABEL,metrics={k:stats([r[k] for r in rows]) for k in ['psnr','ssim','t054_psnr','t054_ssim','delta_psnr','delta_ssim']},best_step_histogram=dict(collections.Counter(str(r['best_step']) for r in rows)),at_step1000=sum(r['best_step']==1000 for r in rows),win_equal_loss={k:dict(win=sum(r['delta_'+k]>0 for r in rows),equal=sum(r['delta_'+k]==0 for r in rows),loss=sum(r['delta_'+k]<0 for r in rows)) for k in ['psnr','ssim']},runtime_seconds=sum(r['seconds'] for r in rows),c_distributions={k:distribution(v) for k,v in fields.items()})
