"""REFERENCE_ORACLE_ONLY: frozen T052 plus fixed local detail; only v changes, exactly1000updates."""
from core import *
...
assert sha(a.preflight/'vs.pt')==pre['vs_sha256'];vs=torch.load(a.preflight/'vs.pt',weights_only=True,map_location='cpu')
...
write(a.out/'config.json',dict(label=LABEL,created_utc=utc(),preflight_sha256=sha(a.preflight/'preflight.json'),split_sha256=SPLIT_SHA,optimizer='fresh Adam; only v',lr_v=.05,coefficient_range=[-1,1],blur='separable ordered5tap [1,4,6,4,1]/16 horizontal then vertical; reflect padding, stride1',grid=[8,8],ev_range=[-2,2],interpolation='raw v bilinear align_corners=False, then tanh',updates_per_image=1000,starts_per_image=1,objective='full RGB MSE float64 through float32 renderer',all_old_coordinates_frozen=True,insertion='frozen T052 y0 -> clamp(y0+tanh(bilinear(v))*(y0-binomial5x5(y0))); inactive exactly y0',selection='earliest strict minimum steps0..1000',gpu=torch.cuda.get_device_name(),torch=torch.__version__,cuda=torch.version.cuda,source_binding=pre['source_binding']))
rows=[]
for i,(r,b) in enumerate(zip(split,pre['rows'])):
    ...
    low=native(a.low_root/r['low']).cuda();assert thash(lifts[i])==b['lift_sha256'];model=Detail(low,starts[i].cuda(),b['gate'],b['box'],lifts[i].cuda(),qs[i].cuda(),us[i].cuda(),bs[i].cuda())
    ...
    assert thash(model.y0)==b['y0_sha256'] and thash(model.detail)==b['basis_sha256']
    assert thash(vs[i])==b['v_sha256'] and thash(model.mask)==b['mask_sha256']
    with torch.no_grad():model.v.copy_(vs[i])
    frozen_buffers={name:value.detach().cpu().clone() for name,value in model.named_buffers()}
    reference=native(a.normal_root/r['normal']).cuda();result=optimize(model,reference,steps=1000)
    assert result['updates']==1000 and torch.equal(model.u.cpu(),us[i]) and torch.equal(model.b.cpu(),bs[i]) and torch.equal(result['v'][0],vs[i]) and torch.equal(model.q.cpu(),qs[i])
    assert torch.equal(model.raw.cpu(),starts[i]) and torch.equal(model.lift.cpu(),lifts[i])
    for name,value in model.named_buffers():assert torch.equal(value.cpu(),frozen_buffers[name]),name
    with torch.no_grad():output=model().cpu();selected_knots=model.knots.cpu();ev=model.ev.cpu();coefficient=model.coefficient().cpu();controls=model.v.tanh().cpu()
    assert torch.isfinite(output).all() and 0<=output.min()<=output.max()<=1 and torch.equal(output[:,:,~model.mask.cpu()],model.y0.cpu()[:,:,~model.mask.cpu()])
    dest=a.out/f'{i:03d}';dest.mkdir();torch.save(dict(v=result['v'],mse=result['mse']),dest/'history.pt');torch.save(dict(label=LABEL,image=output,raw=model.raw.cpu(),lift=model.lift.cpu(),q=model.q.detach().cpu(),mask=model.mask.cpu(),knots=selected_knots,u=model.u.detach().cpu(),b=model.b.detach().cpu(),ev=ev,v=model.v.detach().cpu(),c=coefficient,c_controls=controls,y0=model.y0.cpu(),detail=model.detail.cpu()),dest/'output.pt')
    step=result['best_step'];row=dict(index=i,low=r['low'],best_step=step,initial_mse=float(result['mse'][0]),best_mse=float(result['mse'][step]),final_mse=float(result['mse'][-1]),updates=1000,seconds=time.perf_counter()-begin,raw_sha256=thash(model.raw),lift_sha256=thash(model.lift),q_sha256=thash(model.q),u_sha256=thash(model.u),b_sha256=thash(model.b),v_sha256=thash(model.v),c_sha256=thash(coefficient),c_controls_sha256=thash(controls),y0_sha256=thash(model.y0),detail_sha256=thash(model.detail),ev_sha256=thash(ev),mask_sha256=thash(model.mask),knots_sha256=thash(selected_knots),output_sha256=thash(output),files={n:sha(dest/n) for n in ['history.pt','output.pt']})
