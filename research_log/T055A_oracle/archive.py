...
base=Path('/home/wenchang/asdasdsad/wjq/TTIE');run=base/'runs/20260916-205307-ttie-t055a-oracle';pre=base/'runs/20260916-205215-ttie-t055a-preflight';ev=base/'runs'/a.evaluation_run;out=base/'shared/t055a';out.mkdir(exist_ok=True);backup=Path('/media/wenchang/F/wjq/TTIE/shared/t055a');backup.mkdir(exist_ok=True)
...
config=json.loads((audit/'config.json').read_bytes());release=base/'releases/20260916-205152-ttie-t055a-extension'
for name,h in config['source_binding'].items():assert sha(release/name)==h,name
full=backup/'T055A_full.tar'
...
compact=out/'T055A_compact.tar.gz'
...
(out/'T055A_archives.json').write_text(json.dumps(r,indent=2)+'\n');print(json.dumps(r))
