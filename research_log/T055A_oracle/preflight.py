"""Freeze all100 low-only T054 identities before any normal decode."""
from core import *
...
assert sha(a.accepted/'config.json')=='92eca9a29cb97c01a4644c0a3881235cc8172fb6de4ef1a6f6ebe102fad9c0e2'
...
for i,(r,old,b) in enumerate(zip(split,f['rows'],prior['rows'])):
    ...
    with torch.no_grad():model.v.copy_(saved['v']);output=model().cpu()
    assert torch.equal(model.coefficient().cpu(),saved['c'])
    error=float((output-saved['image']).abs().max());assert error<=1e-6
    rows.append(dict(index=i,low=r['low'],gate=b['gate'],box=b['box'],prior_files=old['files'],prior_best_step=step,raw_sha256=thash(saved['raw']),lift_sha256=thash(saved['lift']),q_sha256=thash(saved['q']),u_sha256=thash(saved['u']),b_sha256=thash(saved['b']),v_sha256=thash(saved['v']),mask_sha256=thash(model.mask),basis_sha256=thash(model.detail),y0_sha256=thash(model.y0),accepted_output_sha256=thash(saved['image']),reconstructed_output_sha256=thash(output),max_abs_error=error,bit_exact=torch.equal(output,saved['image'])))
