...
HERE=Path('research_log/T068D');MODEL=Path('research_log/T066A/evidence/model.json')
...
def endpoint_scores(transfer):
    ...
    for rec,d,end in zip(recs,data,ends):
        ...
        np.testing.assert_array_equal(np.asarray(d['components']),trace['components'][:28].double().numpy())
        ...
        rows.append(dict(**score(d['components'],fs,kl),**identity(rec,trace,d,fs,kl,transfer)))
    assert len(rows)==100
    return rows

def add_ranks(rows):
    return [dict(r,descending_rank=1+sum(x['R_comp']>r['R_comp'] for x in rows),empirical_percentile=sum(x['R_comp']<=r['R_comp'] for x in rows)/len(rows)) for r in rows]

def main(out):
    torch.set_num_threads(1);torch.backends.cuda.matmul.allow_tf32=False;torch.backends.cudnn.allow_tf32=False
    binding=json.loads((HERE/'binding.json').read_bytes())
    for p,h in binding.items():assert sha(p)==h,p
    out.mkdir(parents=True,exist_ok=False);start=time.perf_counter();write(out/'config.json',dict(task='T068-D',source_commit=os.environ['TTIE_SOURCE_COMMIT'],source_binding=binding,started_utc=utc(),optimizer_runs=0,model_fits=0))
    dev=endpoint_scores(False);t99=threshold(dev);write(out/'development_freeze.json',dict(rows=add_ranks(dev),T99_comp=t99,percentile='nearest rank sorted_R[98]',reference_reads=0,frozen_utc=utc()))
    transfer=endpoint_scores(True)
    for r in transfer:r.update(T99_comp=t99,above_T99_comp=r['R_comp']>t99)
    write(out/'transfer_freeze.json',dict(rows=add_ranks(transfer),T99_comp=t99,development_freeze_sha256=sha(out/'development_freeze.json'),reference_reads=0,frozen_utc=utc(),source_commit=os.environ['TTIE_SOURCE_COMMIT']))
    ...
    result=dict(**diagnose(joined),T99_comp=t99,development_above_T99_comp=sum(r['R_comp']>t99 for r in dev),unsafe_rows=[r for r in joined if r['unsafe']],optimizer_runs=0,model_fits=0,completed_utc=utc(),seconds=time.perf_counter()-start)
