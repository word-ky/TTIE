"""T060-D required exact source gate reproduction, before either trajectory."""
import argparse
...
def main(out):
    ...
    for i,r in enumerate(s['anchors']):
        ...
        equal={k:gate[k]==r['gate'][k] for k in gate}
        row=dict(order=i,index=r['index'],bank_index=r['bank_index'],low_hash=thash(low),expected=r['gate'],actual=gate,equal=equal)
        checks.append(row)
        if not all(equal.values()):break
    ok=len(checks)==60 and all(all(r['equal'].values()) for r in checks)
    validate(binding);validate(inputs)
    result=dict(task='T060-D',status='PASS' if ok else 'BLOCKED',started_utc=start,completed_utc=utc(),selection_sha256=sha(selection),source_bindings=binding,inputs=inputs,checks=checks,source_clean_reads=0,reference_gradient_reads=0,metric_reads=0,target_domain_access=0,official_test_access=0,optimizer_steps=0,physical_gpu=1,gpu=torch.cuda.get_device_name(),reason=None if ok else 'Exact recomputed gate differs from frozen T060-B metadata; stopped before any trajectory or source-clean read.')
