"""Independent features/logistic/rollback and offline metric audit; no feedback to fitting."""
...
from research_log.T065B.run import (ROOT, DEV, HERE, DEV_MANIFEST, TARGET_MANIFEST, TARGET,
...
def independent_fit(x,y,normalization):
    ...
    penalty=np.diag([0.]+[.001]*11);beta=np.zeros(12)
...
def check_newton(trace,x,y,normalization,saved):
    ...
    ns=int(y.sum());weights=np.where(y==1,len(y)/(2*ns),len(y)/(2*(len(y)-ns)));penalty=np.diag([0.]+[.001]*11)
    previous=np.zeros(12);max_residual=0.;assert 1<=len(trace)<=50
...
def main(out):
    ...
    labels=(y>=-5.614).astype(np.float64)
    np.testing.assert_array_equal(labels,[row['safe'] for row in table])
    counts=json.loads((out/'class_counts.json').read_bytes());ns=int(labels.sum());nu=len(labels)-ns
    assert counts['safe']==ns and counts['unsafe']==nu and counts['n']==2800
    if not ns or not nu:
        assert result['classification']=='INSUFFICIENT_CLASS_SUPPORT' and not (out/'transfer_freeze.json').exists()
        write(out/'verification.json',dict(status='PASS',classification=result['classification'],development_renders=2800,transfer_renders=0,class_info=counts,verified_utc=utc()));return
    assert counts['safe_weight']==len(labels)/(2*ns) and counts['unsafe_weight']==len(labels)/(2*nu)
    rule=json.loads((out/'selector_manifest.json').read_bytes())
    for name,key in [('model.json','model_sha256'),('training_table.json','training_table_sha256'),('development_features.json','features_sha256'),('development_fit.json','development_fit_sha256'),('newton_trace.json','newton_trace_sha256'),('class_counts.json','class_counts_sha256')]:assert sha(out/name)==rule[key]
    assert sha(NORMALIZATION)==rule['normalization_sha256']
    normalization=json.loads(NORMALIZATION.read_bytes());model=independent_fit(x,labels,normalization);saved=json.loads((out/'model.json').read_bytes());coefficient_error=0.
    ...
    assert saved['coefficient_l2']==rule['coefficient_l2']==.001 and rule['rho']==0.9857470621423519 and saved['threshold']==rule['threshold']==.5
    assert saved['mean']==normalization['mean'] and saved['scale']==normalization['scale'] and saved['class_info']==counts
    residual=check_newton(json.loads((out/'newton_trace.json').read_bytes()),x,labels,normalization,saved)
    ...
    verification=dict(status='PASS',development_renders=2800,development_feature_max_error=feature_error,coefficient_max_error=coefficient_error,
        development_metric_max_error=max(v['independent_error'] for v in quality),training_rows=2800,newton_equation_max_residual=residual,class_info=counts,development_confusion=cm)
    ...
    classification='LINEAR_SAFETY_GUARD_TRANSFER_PASS' if all(g.values()) else 'TRANSFER_NEGATIVE'
