...
from research_log.T065B.core import features,fit,predict,base_choose,choose,summarize,FEATURES,RHO,LAMBDA,class_info,confusion
...
class ReadScope:
    ...
    def hook(self, event, args):
        ...
        raise PermissionError('T065-B target-free read boundary: '+str(p))
...
def main(out):
    ...
    write(out/'config.json', dict(task='T065-B', source_commit=os.environ['TTIE_SOURCE_COMMIT'], source_binding=binding,
    ...
    counts=class_info([r['safe'] for r in training]);write(out/'class_counts.json',counts);print('class_counts',json.dumps(counts),flush=True)
    if not counts['safe'] or not counts['unsafe']:
        write(out/'result.json',dict(classification='INSUFFICIENT_CLASS_SUPPORT',class_info=counts,completed_utc=utc()));return
    normalization=json.loads(NORMALIZATION.read_bytes())
    head,trace=fit([r['features'] for r in training],[r['safe'] for r in training],normalization)
    write(out/'model.json',head);write(out/'newton_trace.json',trace)
    dev_predictions=[predict(r['features'],head).tolist() for r in dev]
    steps=[choose(p,r['base_step']) for p,r in zip(dev_predictions,dev)]
    development=dict(probabilities=dev_predictions,selected_steps=steps,base_steps=[r['base_step'] for r in dev],metrics=summarize(quality,steps),
    ...
    rule=dict(feature_source='research_log/T065A/core.py',feature_source_sha256=binding['research_log/T065A/core.py'],feature_names=FEATURES,feature_definition='exact 11 features in authorization.md; Y=.299R+.587G+.114B; gradient=mean of concatenated absolute valid H and W forward differences; all raw12 coordinates',
        arithmetic='float32 GPU renderer/objective; float64 GPU image/state features and float64 CPU progress and weighted Newton logistic solve; exact T065-A normalization reused; population standard deviations',
        coefficient_l2=LAMBDA,rho=RHO,threshold=.5,rule='keep base if p_safe>=.5; else latest earlier p_safe>=.5; else0',normalization_sha256=sha(NORMALIZATION),newton_trace_sha256=sha(out/'newton_trace.json'),class_counts_sha256=sha(out/'class_counts.json'),
    ...
    result=dict(classification='LINEAR_SAFETY_GUARD_TRANSFER_PASS' if all(metrics['gates'].values()) else 'TRANSFER_NEGATIVE',
