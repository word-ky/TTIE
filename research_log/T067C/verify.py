...
from research_log.T067C.run import EVENTS,TABLE,HERE,PRIOR,MODEL,RULE
from research_log.T066A.verify import independent_predict
import numpy as np

def independent_select(values,probabilities,base):
    reduction=values[0]-min(values);assert reduction>1e-12
    r=[min(1.,max(0.,(values[0]-v)/max(reduction,1e-12))) for v in values];fs=min(k for k in range(base+1) if probabilities[k]>=.5);target=r[fs]+.875*(.9857470621423519-r[fs]);k=min(k for k in range(fs,base+1) if r[k]>=target)
    return dict(k_FS=fs,k_rho=base,r_FS=r[fs],r_target=target,selected_step=k)
...
def main(out):
    ...
    prior=json.loads(PRIOR.read_bytes())['rows'];head=json.loads(MODEL.read_bytes());rule=json.loads(RULE.read_bytes());assert freeze['model_sha256']==sha(MODEL) and freeze['rule_sha256']==sha(RULE) and freeze['prior_sha256']==sha(PRIOR) and freeze['lambda_value']==rule['lambda_value']==.875 and rule['probability_model_sha256']==sha(MODEL);raw,items,recs=load_inputs(True);assert len(rows)==len(events)==100
    with torch.no_grad():
        for i,(row,rec,e) in enumerate(zip(rows,recs,events)):
            seq=[r for r in table if r['index']==i];assert [r['step'] for r in seq]==list(range(28));base=rec['methods']['T063']['selected_step'];prob=independent_predict(prior[i]['features'],head);np.testing.assert_allclose(prob,[r['p_safe'] for r in seq],rtol=0,atol=1e-7);selection=independent_select(prior[i]['totals'],[r['p_safe'] for r in seq],base);k=selection['selected_step']
            for key,value in selection.items():assert abs(row[key]-value)<1e-12
            assert selection['k_FS']==e['first_safe'] and k==row['selected_step'] and row['base_step']==base==e['base_step'] and row['index']==i and row['low']==rec['low']
            assert row['low_sha256']==rec['low_sha256'] and row['trace_sha256']==rec['methods']['T063']['trace_sha256'] and row['p_safe']==seq[k]['p_safe']
            low,trace=state_input(raw,rec,True);np.testing.assert_allclose(prior[i]['totals'],trace['values'][:28],rtol=0,atol=1e-7);state=trace['states'][k];assert thash(state)==row['selected_state_hash'];x=low.cuda();model=CommonRegion2(trace['active']).to(x).eval().requires_grad_(False);model.raw.copy_(state.to(x));image=model(x).cpu();assert thash(image)==row['output_hash']==seq[k]['render_hash']
    ...
    verdict='INTERIOR_PROGRESS_TRANSFER_PASS' if all(gates.values()) else 'INTERIOR_PROGRESS_TRANSFER_NEGATIVE';assert verdict==result['classification']
