...
from research_log.T058AE.storage import atomic_json,save_chunk,reopen,verify_shard0
import traceback
...
selection=selection[1024:]
assert len(selection)==6322 and selection[0]['index']==1024 and selection[-1]['index']==7345
prior_root=Path('/home/wenchang/asdasdsad/wjq/TTIE/runs/20260917-131739-ttie-t058ad-shard0')
prior_archive=Path('research_log/T058AD_archives.json')
prior_files=json.loads(prior_archive.read_bytes())['files']
shard0_before=verify_shard0(prior_root,prior_archive)
...
rows=[];chunks=[];pending=[];pending_indices=[];error=None;failed_index=None;cached_bank=None;bank_hashes=[]
def checkpoint():
    if pending:
        chunks.append(save_chunk(a.out,pending_indices.copy(),pending.copy()));pending.clear();pending_indices.clear()
    atomic_json(a.out/'manifest.json',dict(label='STAGE_A_LEARNED_ENERGY_GRADIENTS_ONLY',complete=False,range=[1024,7345],completed_rows=len(rows),rows=rows,chunks=chunks,selection_sha256=sha(a.out/'selection.json'),updated_utc=utc()))
checkpoint()
try:
    ...
    verification=reopen(a.out,6322,1024)
    shard0_after=verify_shard0(prior_root,prior_archive);assert shard0_before==shard0_after
    atomic_json(a.out/'reopen.json',verification)
    frozen=dict(classification='T058-AE new Stage-A shard frozen; separate combined reopen pending',completed_utc=utc(),range=[1024,7345],processed_once_in_order=True,rows=6322,shard0_before=shard0_before,shard0_after=shard0_after,shard0_recomputed_rows=0,manifest_sha256=sha(a.out/'manifest.json'),selection_sha256=sha(a.out/'selection.json'),independent_reopen=verification,model_hashes_before=initial,model_hashes_after=final,source_bindings=binding,historical_files_unchanged=historical['files'],prior_verifier_files_unchanged=prior_files,source_bank_bindings=bank_hashes,source_jpg_opens=0,source_clean_opens=0,reference_gradient_executions=0,stage_b_executions=0,target_domain_access=0,official_test_access=0,optimizer_updates=0,persistent_scientific_state_changes=0,new_fd_evaluations=0,seconds=time.perf_counter()-start,torch=torch.__version__,cuda=torch.version.cuda,gpu=torch.cuda.get_device_name())
    atomic_json(a.out/'receipt.json',frozen)
    atomic_json(a.out/'new_shard_complete.json',dict(classification=frozen['classification'],rows=6322,range=[1024,7345],receipt_sha256=sha(a.out/'receipt.json'),manifest_sha256=sha(a.out/'manifest.json'),completed_utc=utc()))
    print(frozen['classification'],flush=True)
except Exception as exc:
    error=type(exc).__name__+': '+str(exc)
    (a.out/'error.txt').write_text(traceback.format_exc())
    atomic_json(a.out/'failure.json',dict(classification='T058-AE Stage-A shard incomplete',failed_index=failed_index,error=error,computed_rows=len(rows),committed_rows=sum(x['count'] for x in chunks),complete_marker_exists=(a.out/'new_shard_complete.json').exists(),seconds=time.perf_counter()-start))
    raise
