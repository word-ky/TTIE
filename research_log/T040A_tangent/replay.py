...
assert len(rows)==len(vectors)==f['probes']==14692 and selection['states']==7346
...
expected=[(i,j,g) for i,b in enumerate(selection['banks']) for j in range(b['states']) for g in [1.5,1.75]]
errors=[]
def check(a,b):
...
for actual,published in [(overall,summary['overall'])]+[(aggregates([r for r in rows if r['gain']==g]),summary['by_gain'][str(g)]) for g in [1.5,1.75]]:
    for group,values in actual.items():
        for k,v in values.items():check(v,published[group][k])
high=aggregates([r for r in rows if r['gain']==1.75]);l,g=high['legacy'],high['gain'];yes=l['positive_dot_fraction']-g['positive_dot_fraction']>=.2 and l['cosine_median']-g['cosine_median']>=.25
verdict='source high-gain tangent deficit supported' if yes else 'source high-gain tangent deficit not supported / target-domain state shift remains stronger'
assert verdict==summary['classification']
result=dict(status='PASS',canonical_states=7346,probes=14692,coordinate_groups=44076,scalar_checks=len(errors),max_abs_error=max(errors),classification=verdict,source_targets_opened_after_complete_freeze=True,independent_mask_rebuild=True)
