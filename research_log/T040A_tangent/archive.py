"""Preserve complete T040 source diagnostic runs and compact reviewer evidence."""
from pathlib import Path
import json,hashlib,tarfile,shutil,sys
from datetime import datetime,timezone
home=Path('/home/wenchang/asdasdsad/wjq/TTIE');backup=Path('/media/wenchang/F/wjq/TTIE/shared/t040a');shared=home/'shared/t040a'
...
archives={}
for name,path,full in [('full',backup/'T040A_full.tar',True),('compact',shared/'T040A_compact.tar.gz',False)]:
...
copy=backup/'T040A_compact.tar.gz';shutil.copyfile(shared/'T040A_compact.tar.gz',copy);assert sha(copy)==archives['compact']['sha256']
receipt=dict(completed_utc=datetime.now(timezone.utc).isoformat(),stage_a_400_file_hashes_unchanged=True,archives=archives,compact_backup=str(copy))
for folder in [shared,backup]:(folder/'T040A_archives.json').write_text(json.dumps(receipt,indent=2)+'\n')
