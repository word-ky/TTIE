...
previous=json.loads(Path('research_log/T039A_result/stage_a/selection.json').read_bytes())
baseline=json.loads(Path('research_log/T040A_frozen_baseline.json').read_bytes())
assert sha('research_log/T039A_result/stage_a/selection.json')==baseline['selection_sha256']
assert banks==previous['banks'] and previous['states']==7346
assert sha('research_log/T039A_result/stage_b/summary.json')==baseline['summary_sha256']
...
for index,entry in enumerate(banks):
    ...
    write(a.out/'progress.json',dict(completed_banks=len(rows),completed_probes=count,utc=utc()));print(f'{index+1}/400 banks; {count}/14692 probes frozen',flush=True)
assert count==14692 and model_hashes()==initial and all(p.grad is None for m in [head,scorer] for p in m.parameters())
