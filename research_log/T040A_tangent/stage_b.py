...
assert f['probes']==14692 and f['source_target_opens']==f['optimizer_updates']==f['selection_changes']==0
...
assert len(rows)==14692 and len(opened)==80 and all(r['utc']>f['completed_utc'] for r in opened)
summary=dict(label=LABEL,canonical_states=7346,probes=14692,source_ids=80,overall=aggregate(rows),by_gain={str(g):aggregate([r for r in rows if r['gain']==g]) for g in GAINS})
summary['frozen_T039_gain125_baseline']=json.loads(Path('research_log/T040A_frozen_baseline.json').read_bytes());summary['classification']=classify(summary['by_gain']['1.75']);write(a.out/'states.json',rows);write(a.out/'vectors.json',vectors);write(a.out/'summary.json',summary)
for row in f['rows']:assert sha(a.stage_a/row['file'])==row['sha256']
