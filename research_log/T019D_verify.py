"""Independent T019-D replay: reference-free CPU decisions, then table arithmetic."""
import argparse
...
def metrics(root, cohort, selected, evaluation, replay_receipt, output):
    ...
    bindings=dict(exclusions_sha256=root/'research_log/T019D_exclusions.json',manifest_sha256=cohort/'manifest.json',
    ...
    lock=read(root/'research_log/T019D_pipeline_lock.json')
    assert digest(root/'research_log/T019D_pipeline_lock.json')==config['pipeline_lock_sha256']
    ...
    prior=read(root/'research_log/T019D_exclusions.json');manifest=read(cohort/'manifest.json');mf=read(cohort/'manifest_frozen.json')
    union=set()
        ...
        def visit(v):
            if isinstance(v,dict):
                for k,x in v.items():
                    ...
                    elif k in ('image_ids','excluded_prior_ids','excluded_ids','used_ids','previously_inspected_extra_ids','development_T016_T018_ids','train_image_ids','heldout_image_ids') and isinstance(x,list):found.update(i for i in x if isinstance(i,int))
                    else:visit(x)
    ...
    assert digest(cohort/'manifest.json')==mf['manifest_sha256'] and digest(root/'research_log/T019D_exclusions.json')==mf['exclusions_sha256']
    ...
    expected_mapping=[dict(row_index=i*3+j,image_id=e['image_id'],filename=e['filename'],condition=c,source_sha256=e['sha256'])
        for i,e in enumerate(manifest['images']) for j,c in enumerate(('left_right','quadrants','offset_left_right_40'))]
