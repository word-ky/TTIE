...
for i,(r,old,b) in enumerate(zip(split,f['rows'],prior['rows'])):
    ...
    assert sha(a.low_root/r['low'])==r['low_sha256'];low=native(a.low_root/r['low']).cuda();model=Spatial(low,saved['raw'].cuda(),b['gate'],b['box'],saved['lift'].cuda(),saved['q'].cuda(),saved['u'].cuda(),saved['b'].cuda())
    with torch.no_grad():output=model().cpu()
    error=float((output-saved['image']).abs().max());assert error<=1e-6
    rows.append(dict(index=i,low=r['low'],gate=b['gate'],box=b['box'],prior_files=old['files'],prior_best_step=step,raw_sha256=thash(saved['raw']),lift_sha256=thash(saved['lift']),q_sha256=thash(saved['q']),u_sha256=thash(saved['u']),b_sha256=thash(saved['b']),accepted_output_sha256=thash(saved['image']),reconstructed_output_sha256=thash(output),max_abs_error=error,bit_exact=torch.equal(output,saved['image'])))
