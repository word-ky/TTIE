...
base=Path('/home/wenchang/asdasdsad/wjq/TTIE');run=base/'runs/20260916-190122-ttie-t053a-oracle';pre=base/'runs/20260916-185805-ttie-t053a-preflight';ev=base/'runs'/a.evaluation_run;out=base/'shared/t053a';out.mkdir(exist_ok=True);backup=Path('/media/wenchang/F/wjq/TTIE/shared/t053a');backup.mkdir(exist_ok=True)
...
config=json.loads((audit/'config.json').read_bytes());release=base/'releases/20260916-185445-ttie-t053a-range'
for name,h in config['source_binding'].items():assert sha(release/name)==h,name
full=backup/'T053A_full.tar'
...
compact=out/'T053A_compact.tar.gz'
...
(out/'T053A_archives.json').write_text(json.dumps(r,indent=2)+'\n');print(json.dumps(r))
