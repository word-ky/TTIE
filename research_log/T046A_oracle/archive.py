...
base=Path('/home/wenchang/asdasdsad/wjq/TTIE');run=base/'runs/20260915-173335-ttie-t046a-oracle';pre=base/'runs/20260915-173248-ttie-t046a-preflight';ev=base/'runs'/a.evaluation_run;out=base/'shared/t046a';out.mkdir(exist_ok=True);backup=Path('/media/wenchang/F/wjq/TTIE/shared/t046a');backup.mkdir(exist_ok=True)
...
config=json.loads((audit/'config.json').read_bytes());release=base/'releases/20260915-173225-ttie-t046a-extension'
for name,h in config['source_binding'].items():assert sha(release/name)==h,name
full=backup/'T046A_full.tar'
...
compact=out/'T046A_compact.tar.gz'
...
(out/'T046A_archives.json').write_text(json.dumps(r,indent=2)+'\n');print(json.dumps(r))
