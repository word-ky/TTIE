...
with zipfile.ZipFile(archive) as z:
    ...
    selected = sorted(pool, key=lambda p: hashlib.sha256(
        ('TTIE-T027B-smoke|' + p).encode()).hexdigest())[:8]
    rows = []
    for name in selected:
        ...
        rows.append({'low': name, 'sha256': hashlib.sha256(data).hexdigest(),
                     'selection_hash': hashlib.sha256(('TTIE-T027B-smoke|' + name).encode()).hexdigest()})
receipt = {'task': 'T027-B', 'selection_prefix': 'TTIE-T027B-smoke|',
