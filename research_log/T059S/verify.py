...
def main(out):
    ...
    for action,row in zip(actions,rows):
        ...
        assert abs(mse0-row['mse0'])<1e-14 and abs(mse1-row['mse1'])<1e-14
        rel=(mse1-mse0)/mse0 if mse0 else 0;assert abs(rel-row['relative_mse_change'])<1e-12
        checks.append(dict(bank_index=row['bank_index'],adam_max_error=verr,detail_max_error=derr,coefficient_max_error=cerr,render_max_error=yerr,mse0=mse0,mse1=mse1))
    ...
    for k,v in dict(mean_relative_change=float(delta.mean()),median_relative_change=float(np.median(delta)),p90_relative_harm=float(np.quantile(np.maximum(delta,0),.9)),max_relative_harm=float(np.maximum(delta,0).max())).items():assert abs(v-expected[k])<1e-12
