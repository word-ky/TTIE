"""Freeze all100 low-only T047 identities before any normal decode."""
from core import *
...
for i,(r,old,b) in enumerate(zip(split,f['rows'],prior['rows'])):
    ...
    assert step==old['best_step'] and torch.equal(saved['lift'],hist['lift'][step]) and thash(saved['raw'])==b['raw_sha256'] and thash(saved['raw'])==old['raw_sha256'] and thash(saved['image'])==old['output_sha256']
    assert sha(a.low_root/r['low'])==r['low_sha256'];low=native(a.low_root/r['low']).cuda();model=Affine(low,saved['raw'].cuda(),b['gate'],b['box'],saved['lift'].cuda())
