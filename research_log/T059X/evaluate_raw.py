...
validate_inputs(json.loads(Path('research_log/T059X/evaluation_repair_binding.json').read_bytes()))
...
first=json.loads((out/'reference_open.json').read_bytes())['first_reference_read_utc'];assert first>f['utc']
...
for r,s in zip(records,pairing['selected']):
    ...
    t=torch.load(out/r['file'],weights_only=True,map_location='cpu');raw=native_rgb(r['image_file'])
    row={k:r[k] for k in ['index','low','norm','active_regions','acted','sha256']}
    for name,key in [('raw','y0'),('ours','y1')]:
        im=(raw if name=='raw' else t[key]).squeeze(0).permute(1,2,0).numpy().astype(float);assert im.shape==normal.shape and np.isfinite(im).all();mse=float(np.mean((im-normal)**2));row[name+'_mse']=mse;row[name+'_psnr']=-10*math.log10(max(mse,1e-12));row[name+'_ssim']=rgb_ssim(im,normal)
