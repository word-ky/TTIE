...
def main(out):
    ...
    for r,t,c,s in zip(records,table,cohort['anchors'],pairing['selected']):
        ...
        b=torch.load(out/r['file'],map_location='cpu',weights_only=True);raw=native_rgb(c['image_file']);assert all(torch.isfinite(z).all() for z in b.values())
        h,w=raw.shape[-2:];mask=b['mask'].reshape(h,w);active=mask[[0,0,h//2,h//2],[0,w//2,0,w//2]]
        with torch.no_grad():replayed=CommonRegion2(active.cuda()).cuda()(raw.cuda()).cpu()
        assert torch.equal(replayed,b['y0']), 'Inherited W zero-state ISP replay mismatch'
        ...
        for name,key in [('raw','y0'),('ours','y1')]:
            im=(raw if name=='raw' else b[key]).squeeze(0).permute(1,2,0).numpy().astype(float);m=float(torch.from_numpy(im-normal).square().mean());vals=dict(mse=m,psnr=-10*math.log10(max(m,1e-12)),ssim=independent_ssim(im,normal))
