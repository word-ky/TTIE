...
binding=json.loads(Path('research_log/T059X/source_binding.json').read_bytes());selection=json.loads(Path('research_log/T059X/cohort.json').read_bytes());inputs=selection['inputs']
allowed=set(inputs)|{str(Path(n).resolve()) for n in binding}|{str(Path('research_log/T059X/source_binding.json').resolve())}
install_firewall(ROOT,allowed,out)
validate_inputs(binding);validate_inputs(inputs)
sf=json.loads(Path('research_log/T059X/cohort_freeze.json').read_bytes());assert sha('research_log/T059X/cohort.json')==sf['sha256'];assert sf['utc']<utc()
...
for r in selection['anchors']:
    low=native_rgb(r['image_file']).cuda()
