"""Freeze all100 low-only T051 identities before any normal decode."""
from core import *
...
assert sha(a.accepted/'config.json')=='1766e7bb46e77028b043b6c99176b800b3ccd940149d4b5321d23acb46503b3f'
...
o.Image.open=low_only;a.out.mkdir(parents=True,exist_ok=False);rows=[];starts=[];lifts=[];qs=[];us=[]
for i,(r,old,b) in enumerate(zip(split,f['rows'],prior['rows'])):
    ...
    assert step==old['best_step'] and torch.equal(saved['u'],hist['u'][step]) and thash(saved['u'])==old['u_sha256'] and thash(saved['q'])==old['q_sha256'] and thash(saved['raw'])==b['raw_sha256']==old['raw_sha256'] and thash(saved['lift'])==b['lift_sha256']==old['lift_sha256'] and thash(saved['image'])==old['output_sha256']
    assert sha(a.low_root/r['low'])==r['low_sha256'];low=native(a.low_root/r['low']).cuda();model=Spatial(low,saved['raw'].cuda(),b['gate'],b['box'],saved['lift'].cuda(),saved['q'].cuda(),saved['u'].cuda())
    with torch.no_grad():output=model().cpu()
    error=float((output-saved['image']).abs().max());assert error<=1e-6
    rows.append(dict(index=i,low=r['low'],gate=b['gate'],box=b['box'],prior_files=old['files'],prior_best_step=step,raw_sha256=thash(saved['raw']),lift_sha256=thash(saved['lift']),q_sha256=thash(saved['q']),u_sha256=thash(saved['u']),accepted_output_sha256=thash(saved['image']),reconstructed_output_sha256=thash(output),max_abs_error=error,bit_exact=torch.equal(output,saved['image'])))
    starts.append(saved['raw'].clone());lifts.append(saved['lift'].clone());qs.append(saved['q'].clone());us.append(saved['u'].clone())
assert len(opened)==100
torch.save(torch.stack(us),a.out/'us.pt');torch.save(torch.stack(starts),a.out/'starts.pt');torch.save(torch.stack(lifts),a.out/'lifts.pt');torch.save(torch.stack(qs),a.out/'qs.pt');write(a.out/'preflight.json',dict(label=LABEL,completed_utc=utc(),rows=rows,normal_decodes=0,opened_lows=opened,starts_sha256=sha(a.out/'starts.pt'),lifts_sha256=sha(a.out/'lifts.pt'),qs_sha256=sha(a.out/'qs.pt'),us_sha256=sha(a.out/'us.pt'),prior_freeze_sha256=PRIOR_FREEZE,prior_preflight_sha256=PRIOR_PREFLIGHT,split_sha256=SPLIT_SHA,source_binding=binding,source_binding_sha256=sha(a.binding),all100_max_abs=max(r['max_abs_error'] for r in rows),all100_bit_exact=all(r['bit_exact'] for r in rows),seconds=time.perf_counter()-start))
