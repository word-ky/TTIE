"""REFERENCE_ORACLE_ONLY: joint smooth exposure and additive fields; all pre-T051 coordinates frozen, exactly500updates."""
from core import *
...
write(a.out/'config.json',dict(label=LABEL,created_utc=utc(),preflight_sha256=sha(a.preflight/'preflight.json'),split_sha256=SPLIT_SHA,optimizer='fresh Adam; only u+b',lr_u=.05,lr_b=.01,b_range=[-.2,.2],b_projection='physical controls clamped after each update',grid=[8,8],ev_range=[-2,2],interpolation='bilinear align_corners=False after2tanh',updates_per_image=500,starts_per_image=1,objective='full RGB MSE float64 through float32 renderer',all_old_coordinates_frozen=True,insertion='affine clamp -> multiply by2**e + bilinear b -> clamp -> frozen tone -> hard gate',selection='earliest strict minimum steps0..500',gpu=torch.cuda.get_device_name(),torch=torch.__version__,cuda=torch.version.cuda,source_binding=pre['source_binding']))
rows=[]
for i,(r,b) in enumerate(zip(split,pre['rows'])):
    ...
    low=native(a.low_root/r['low']).cuda();assert thash(lifts[i])==b['lift_sha256'];model=Spatial(low,starts[i].cuda(),b['gate'],b['box'],lifts[i].cuda(),qs[i].cuda(),us[i].cuda())
    assert thash(qs[i])==b['q_sha256']
    assert torch.equal(model.q.cpu(),qs[i]) and thash(model.u)==b['u_sha256']
    reference=native(a.normal_root/r['normal']).cuda();result=optimize(model,reference,steps=500)
    assert result['updates']==500 and torch.equal(result['u'][0],us[i]) and torch.count_nonzero(result['b'][0])==0 and torch.equal(model.q.cpu(),qs[i])
    ...
    dest=a.out/f'{i:03d}';dest.mkdir();torch.save(dict(u=result['u'],b=result['b'],mse=result['mse']),dest/'history.pt');torch.save(dict(label=LABEL,image=output,raw=model.raw.cpu(),lift=model.lift.cpu(),q=model.q.detach().cpu(),knots=selected_knots,u=model.u.detach().cpu(),b=model.b.detach().cpu(),ev=ev,offset=offset),dest/'output.pt')
