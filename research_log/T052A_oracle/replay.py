...
assert sha(a.accepted/'freeze.json')==pre['prior_freeze_sha256']=='d66edca57c6cac1ffe15a428552c73e2e343363165a2808f16a1b01cce460667'
assert sha(a.accepted/'pairs.json')==receipt['prior_csv_sha256']=='ecc607ac2c04a21d5ad53c06264f22869e0ee3f65810c22d952b5bbec665be2f'
assert sha(a.split)==pre['split_sha256'];split=read(a.split)['selected'];baseline=read(a.accepted/'pairs.json')
starts={}
for name,key in [('starts','starts_sha256'),('lifts','lifts_sha256'),('qs','qs_sha256'),('us','us_sha256')]:
...
errors=[];render_errors=[];field_errors=[];field_values=[];offset_values=[];b_values=[];steps=[];values={k:[] for k in ['psnr','ssim','t051_psnr','t051_ssim','t050_psnr','t050_ssim','total_psnr','total_ssim','delta_psnr','delta_ssim']}
for i,(row,item,old,main) in enumerate(zip(f['rows'],split,baseline,pairs)):
 ...
 assert u.shape==(501,1,1,8,8) and hist['mse'].shape==(501,) and torch.isfinite(u).all() and torch.isfinite(hist['mse']).all() and torch.equal(u[0],starts['us'][i]) and hist['b'].shape==(501,1,1,8,8) and torch.count_nonzero(hist['b'][0])==0 and torch.isfinite(hist['b']).all() and hist['b'].min()>=-.2 and hist['b'].max()<=.2
 assert (2*u.tanh()).min()>=-2 and (2*u.tanh()).max()<=2
 step=min(range(501),key=lambda j:float(hist['mse'][j]));steps.append(step)
 assert step==row['best_step']==main['best_step'] and torch.equal(v['u'],u[step]) and torch.equal(v['b'],hist['b'][step])
 ...
 field_values.extend(v['ev'].numpy()[0,0][mask].tolist());offset_values.extend(v['offset'].numpy()[0,0][mask].tolist());b_values.extend(v['b'].numpy().flatten().tolist())
 ...
 calculated=dict(psnr=psnr,ssim=ssim,t050_psnr=old['t050_psnr'],t050_ssim=old['t050_ssim'],total_psnr=psnr-old['t050_psnr'],total_ssim=ssim-old['t050_ssim'],t051_psnr=old['psnr'],t051_ssim=old['ssim'],delta_psnr=psnr-old['psnr'],delta_ssim=ssim-old['ssim'])
...
for name,data in [('offset_distribution',offset_values),('b_control_distribution',b_values)]:
 fv=summary[name];bound=float(np.float32(.2));assert fv['count']==len(data) and fv['lower_hits']==sum(x<=-bound for x in data) and fv['upper_hits']==sum(x>=bound for x in data)
...
for k in ['psnr','ssim']:
 data=values['total_'+k];assert summary['total_win_equal_loss'][k]==dict(win=sum(x>0 for x in data),equal=sum(x==0 for x in data),loss=sum(x<0 for x in data))
classification='spatial affine coupling capacity supported' if statistics.mean(values['total_psnr'])>=1.5 and statistics.median(values['total_psnr'])>=.75 and statistics.mean(values['total_ssim'])>=0 else 'not supported under fixed probe'
