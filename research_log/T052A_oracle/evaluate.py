...
o.Image.open=normal_only;rows=[];field_values=[];offset_values=[];b_values=[];starts=torch.load(a.preflight/'starts.pt',weights_only=True,map_location='cpu');initial_lifts=torch.load(a.preflight/'lifts.pt',weights_only=True,map_location='cpu')
for i,(item,row,baseline) in enumerate(zip(split,f['rows'],old)):
    ...
    assert hist['b'].shape==(501,1,1,8,8) and hist['b'].min()>=-.2 and hist['b'].max()<=.2 and torch.count_nonzero(hist['b'][0])==0 and hist['u'].shape==(501,1,1,8,8) and hist['mse'].shape==(501,)
    index=int(hist['mse'].argmin());assert index==row['best_step'] and torch.equal(saved['u'],hist['u'][index]) and torch.equal(saved['b'],hist['b'][index]) and thash(saved['q'])==pre['rows'][i]['q_sha256'] and torch.equal(saved['lift'],initial_lifts[i]) and thash(saved['raw'])==pre['rows'][i]['raw_sha256'] and thash(saved['lift'])==row['lift_sha256'] and thash(saved['q'])==row['q_sha256'] and thash(saved['knots'])==row['knots_sha256']
    ...
    mask=torch.tensor(pre['rows'][i]['gate']['active']).reshape(2,2)[(torch.arange(saved['ev'].shape[-2])>=saved['ev'].shape[-2]//2).long()[:,None],(torch.arange(saved['ev'].shape[-1])>=saved['ev'].shape[-1]//2).long()[None,:]]
    field_values.extend(saved['ev'][0,0][mask].flatten().tolist());offset_values.extend(saved['offset'][0,0][mask].flatten().tolist());b_values.extend(saved['b'].flatten().tolist())
    rows.append(dict(index=i,low=item['low'],best_step=index,psnr=v['psnr'],ssim=v['ssim'],t050_psnr=float(baseline['t050_psnr']),t050_ssim=float(baseline['t050_ssim']),total_psnr=v['psnr']-float(baseline['t050_psnr']),total_ssim=v['ssim']-float(baseline['t050_ssim']),t051_psnr=float(baseline['psnr']),t051_ssim=float(baseline['ssim']),delta_psnr=v['psnr']-float(baseline['psnr']),delta_ssim=v['ssim']-float(baseline['ssim']),seconds=row['seconds']))
def stats(v):return dict(mean=float(np.mean(v)),median=float(np.median(v)))
s=dict(label=LABEL,metrics={k:stats([r[k] for r in rows]) for k in ['psnr','ssim','t051_psnr','t051_ssim','t050_psnr','t050_ssim','total_psnr','total_ssim','delta_psnr','delta_ssim']},best_step_histogram=dict(collections.Counter(str(r['best_step']) for r in rows)),at_step500=sum(r['best_step']==500 for r in rows),win_equal_loss={k:dict(win=sum(r['delta_'+k]>0 for r in rows),equal=sum(r['delta_'+k]==0 for r in rows),loss=sum(r['delta_'+k]<0 for r in rows)) for k in ['psnr','ssim']},runtime_seconds=sum(r['seconds'] for r in rows))
...
s['total_win_equal_loss']={k:dict(win=sum(r['total_'+k]>0 for r in rows),equal=sum(r['total_'+k]==0 for r in rows),loss=sum(r['total_'+k]<0 for r in rows)) for k in ['psnr','ssim']}
for name,values in [('offset_distribution',offset_values),('b_control_distribution',b_values)]:
    bound=float(np.float32(.2));s[name]=dict(count=len(values),min=min(values),max=max(values),mean=float(np.mean(values)),median=float(np.median(values)),lower_hits=sum(v<=-bound for v in values),upper_hits=sum(v>=bound for v in values))
s['classification']=verdict(s['metrics']['total_psnr']['mean'],s['metrics']['total_psnr']['median'],s['metrics']['total_ssim']['mean'])
