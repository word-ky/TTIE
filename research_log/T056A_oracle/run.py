...
write(a.out/'config.json',dict(label=LABEL,created_utc=utc(),preflight_sha256=sha(a.preflight/'preflight.json'),split_sha256=SPLIT_SHA,optimizer='fresh Adam; only w',lr=.05,updates_per_image=500,starts_per_image=1,grid=[8,8],coefficient_range=[-1,1],interpolation='raw w bilinear align_corners=False then tanh',B5=[1,4,6,4,1],B5_denominator=16,B9=[1,8,28,56,70,56,28,8,1],B9_denominator=256,padding='reflect',operator='D2=B5(y0)-B9(y0); clamp(frozenT055+c2*D2) on active mask; inactive frozenT055',objective='full RGB float64 MSE through float32 renderer',selection='earliest strict minimum states0..500',gpu=torch.cuda.get_device_name(),torch=torch.__version__,cuda=torch.version.cuda,source_binding=pre['source_binding']))
rows=[]
for i,(item,b) in enumerate(zip(split,pre['rows'])):
 ...
 assert sha(a.low_root/item['low'])==item['low_sha256'];low=native(a.low_root/item['low']).cuda();m=Band(low,v['raw'].cuda(),b['gate'],b['box'],v['lift'].cuda(),v['q'].cuda(),v['u'].cuda(),v['b'].cuda(),v['v'].cuda())
 assert torch.equal(m.one.cpu(),start) and torch.equal(start,v['image']) and thash(m.d2)==b['d2_sha256']
 ...
 assert hist['updates']==500 and torch.count_nonzero(hist['w'][0])==0
 for name,t in m.named_buffers():assert torch.equal(t.cpu(),frozen[name]),name
 with torch.no_grad():image=m().cpu();c2=m.coefficient().cpu();controls=m.w.tanh().cpu()
 assert torch.isfinite(image).all() and torch.equal(image[:,:,~m.mask.cpu()],start[:,:,~m.mask.cpu()])
 saved={name:getattr(m,name).detach().cpu() for name in OLD+['one','d2','w']};saved.update(image=image,c2=c2,c2_controls=controls,label=LABEL)
 dest=a.out/f'{i:03d}';dest.mkdir();torch.save(saved,dest/'output.pt');torch.save(dict(w=hist['w'],mse=hist['mse']),dest/'history.pt')
