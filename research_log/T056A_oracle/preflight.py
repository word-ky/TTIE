...
for i,(r,old,b) in enumerate(zip(split,f['rows'],prior['rows'])):
    ...
    assert sha(a.low_root/r['low'])==r['low_sha256'];low=native(a.low_root/r['low']).cuda();model=Band(low,saved['raw'].cuda(),b['gate'],b['box'],saved['lift'].cuda(),saved['q'].cuda(),saved['u'].cuda(),saved['b'].cuda(),saved['v'].cuda())
