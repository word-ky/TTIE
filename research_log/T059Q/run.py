...
def evaluate(out):
 ...
 for b in table:
  assert b['E_state']==prior[b['bank']]['predicted_argmin_state_index']
 ...
 write(out/'evaluation_table.json',table);before={**inputs,str(F):sha(F)};after={p:sha(p) for p in before};assert before==after and all(sha(p)==h for p,h in own.items());assert sha(out/'decisions.json')==mark['decisions_sha256'];result=dict(task='T059-Q',status='DONE',classification=label,coverage=d['coverage'],statistics=stats,comparative_conditions=criteria,prior_F2_tail= tails,marker=mark,target_opened_utc=opened,target_rows_sha256=sha(out/'target_rows.json'),evaluation_table_sha256=sha(out/'evaluation_table.json'),inputs_before=before,inputs_after=after,authorization='7a1f070ef1a25c13b40820a47f9797b203585c9d',source=json.loads((HERE/'publication.json').read_text())['source'],decision_source='8473e9a55e0c6ec271775731ad1127ca5ff1c860',decision_source_release=str(ROOT/'releases/20260918-ttie-t059q-consensus'),first_target_access=json.loads((ROOT/'runs/20260918-183141-ttie-t059q-consensus/artifacts/T059Q/target_rows.json').read_text())['opened_utc'],historical_F2_max_regret_roundoff=max(abs(b['policies']['E']['regret']-prior[b['bank']]['argmin_regret']) for b in table),metric_precision='target differences FP32 as persisted; summaries float64; linear p90; exact oracle hit means target equals minimum including ties',counters={k:0 for k in ['training_runs','optimizer_steps','model_forwards','new_feature_forwards','source_target_reads_before_decision_freeze','outer_supervision_reads','target_domain_access','lolv2_access','official_test_access','inference_reference_leakage']},completed_utc=utc());write(out/'result.json',result);print(json.dumps(dict(classification=label,coverage=d['coverage'],statistics=stats,conditions=criteria)),flush=True)
