"""Reference gradients only after all200 low-only probes are frozen."""
from core import *
...
assert f['audited_states']==200 and f['normal_decodes']==f['prior_metric_files_opened']==f['prior_loss_id_files_opened']==f['optimizer_updates']==f['selection_changes']==0
...
cohort=json.loads(a.manifest.read_bytes())['selected'];baselines=json.loads(Path('research_log/T041A_source_baselines.json').read_bytes())
for gain,v in baselines.items():
    assert sha(v['path'])==v['sha256']
    assert json.loads(Path(v['path']).read_bytes())['by_gain'][gain]==v['groups']
...
for item,bound in zip(cohort,f['rows']):
    ...
    for j,gain in enumerate(GAINS):
        raw=saved['states'][j];assert torch.equal(raw[:,:2],saved['legacy'])
        ...
        rows.append(dict(index=bound['index'],low=item['low'],selected_step=bound['selected_step'],gain=gain,raw=raw.tolist(),g_e=g_e.tolist(),g_r=g_r.tolist(),energy=saved['energies'][j],output_sha256=thash(output),masks={k:v.tolist() for k,v in saved['masks'].items()},groups={k:alignment(g_e,g_r,v) for k,v in saved['masks'].items()}))
    print(f'{bound["index"]+1}/100 reference gradients',flush=True)
assert len(rows)==200
summary=dict(label=LABEL,audited_states=200,fixed_gains=GAINS,by_gain={str(g):aggregate([r for r in rows if r['gain']==g]) for g in GAINS},source_baselines=baselines)
summary['target_minus_source']={gain:{group:{k:summary['by_gain'][gain][group][k]-v['groups'][group][k] for k in ['positive_dot_fraction','cosine_median']} for group in ['legacy','gain','total']} for gain,v in baselines.items()}
summary['classification']=classify(summary['by_gain']['1.75']['total'])
