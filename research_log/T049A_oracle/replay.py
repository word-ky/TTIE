...
assert sha(a.accepted/'freeze.json')==pre['prior_freeze_sha256'] and sha(a.accepted/'pairs.json')==receipt['prior_csv_sha256']=='29a33623bbcfc8d9500254344173b68556a6187ce15b1115a435b53fb2a18b48'
...
errors=[];values={k:[] for k in ['psnr','ssim','t048_psnr','t048_ssim','delta_psnr','delta_ssim']};steps=[];segments=[];knot_values=[];render_errors=[];initial_lifts=torch.load(a.preflight/'lifts.pt',weights_only=True,map_location='cpu');assert sha(a.preflight/'lifts.pt')==pre['lifts_sha256']
for i,(row,item,old,main) in enumerate(zip(f['rows'],split,baseline,pairs)):
    ...
    assert h['q'].shape==(501,4,8) and h['mse'].shape==(501,) and torch.isfinite(h['q']).all() and torch.isfinite(h['mse']).all()
    step=min(range(501),key=lambda j:float(h['mse'][j]));steps.append(step)
    assert step==row['best_step']==main['best_step'] and torch.equal(v['q'],h['q'][step]) and torch.equal(starts[i],v['raw']) and torch.equal(initial_lifts[i],v['lift']) and torch.count_nonzero(h['q'][0])==0
    ...
    assert np.isfinite(all_knots).all() and np.all(np.diff(all_knots,axis=-1)>=0) and np.max(np.abs(all_knots[0]-np.arange(9)/8))==0
    ...
    val=dict(psnr=psnr,ssim=ssim,t048_psnr=float(old['psnr']),t048_ssim=float(old['ssim']),delta_psnr=psnr-float(old['psnr']),delta_ssim=ssim-float(old['ssim']))
...
classification='SOTA-scale monotonic-tone capacity supported' if statistics.mean(values['delta_psnr'])>=2 and statistics.median(values['delta_psnr'])>=1 else 'SOTA-scale monotonic-tone capacity not supported under fixed probe'
