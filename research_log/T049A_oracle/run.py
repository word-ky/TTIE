"""REFERENCE_ORACLE_ONLY: tone only; all T048 coordinates frozen, exactly500updates."""
from core import *
...
write(a.out/'config.json',dict(label=LABEL,created_utc=utc(),preflight_sha256=sha(a.preflight/'preflight.json'),preflight_completed_utc=pre['completed_utc'],split_sha256=SPLIT_SHA,optimizer='fresh Adam at identity tone on exact T048 winner',lr=.03,segments=8,knots_x=[i/8 for i in range(9)],parameterization='normalized softplus increments, q=0 identity, endpoints0/1',updates_per_image=500,starts_per_image=1,objective='full RGB MSE float64 accumulation through frozen float32 renderer plus lift',lift_bounds=[-.2,.2],all_t048_coordinates_frozen=True,insertion='common gain -> additive lift -> accepted identity contrast arithmetic -> clamp -> monotonic tone LUT -> hard gate compositing',selection='earliest strict minimum over steps0..500',gpu=torch.cuda.get_device_name(),torch=torch.__version__,cuda=torch.version.cuda,source_binding=pre['source_binding']))
rows=[]
for i,(r,b) in enumerate(zip(split,pre['rows'])):
    ...
    assert result['updates']==500 and torch.count_nonzero(result['q'][0])==0 and torch.count_nonzero(result['q'][:,~model.active.flatten().cpu()])==0
    ...
    step=result['best_step'];row=dict(index=i,low=r['low'],best_step=step,initial_mse=float(result['mse'][0]),best_mse=float(result['mse'][step]),final_mse=float(result['mse'][-1]),updates=500,seconds=time.perf_counter()-begin,raw_sha256=thash(model.raw),lift_sha256=thash(model.lift),q_sha256=thash(model.q),knots_sha256=thash(selected_knots),output_sha256=thash(output),files={n:sha(dest/n) for n in ['history.pt','output.pt']})
