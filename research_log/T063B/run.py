...
from research_log.T063B.core import losses, ratios, choose, calibrate, summarize
...
class ReadScope:
    ...
    def hook(self, event, args):
        ...
        raise PermissionError('T063-B target-free read boundary: '+str(p))
...
def render_components(low, trace):
    ...
    with torch.no_grad():
        for state in trace['states'][:28]:
            ...
            components.append(losses(x, y).cpu().numpy().astype(np.float64).tolist())
...
def reconstruct(out, transfer=False, tau=None):
    ...
    with scope:
        for i, (item, rec) in enumerate(zip(items, recs)):
            ...
            ratio = ratios(parts); assert np.isfinite(ratio).all()
            row = dict(index=i, low=item['low'], components=parts, ratios=ratio.tolist(), hashes=hashes)
            if transfer:
                k = choose(ratio, tau); row.update(selected_step=k, output_hash=hashes[k])
...
def main(out):
    ...
    write(out/'config.json', dict(task='T063-B', source_commit=os.environ['TTIE_SOURCE_COMMIT'], source_binding=binding,
    ...
    table, best, passed = calibrate([r['ratios'] for r in dev], quality)
    write(out/'threshold_candidates.json', table)
    with (out/'threshold_candidates.csv').open('x', newline='') as f:
        fields=['tau','mean_delta_psnr','median_delta_psnr','regressions_t026','worst_delta_t026','mean_delta_ssim','eligible']
        writer=csv.DictWriter(f,fieldnames=fields,extrasaction='ignore');writer.writeheader();writer.writerows(table)
    rule = dict(rule='largest k in 1..27 with finite R_k <= tau, else 0', statistic='(S_k-S_0)/max(P_0-P_k,1e-8); P=10*exp+5*col',
                arithmetic='accepted float32 components; binary64 ratio and tau', sentinel='math.nextafter(min,-inf),math.nextafter(max,+inf)',
