"""Independently verify dataset-generic MR. Illuminate low-only outputs (T073-D).
...
artifact, cross-checks manifest rows against per-image decision.json records
and the frozen low receipt, checks each output's geometry against the low
image's own PIL size, and checks that each native float lies inside the
truncation bin of the official uint8 image captured with it (a consistency
check between the method's own two outputs, not a quality measurement).
"""
...
BIN_SLACK = 1e-3  # |255 * float32(v / 255) - v| is at most ~3e-5 for v in [0, 255]
...
def bin_violations(tensor_rgb, official_bgr):
    """Count values outside [u8, u8 + 1) of the official truncation of clip(v, 0, 255)."""
    scaled = np.clip(tensor_rgb[0].numpy().transpose(1, 2, 0).astype(np.float64) * 255.0, 0.0, 255.0)
...
def map_back(native_tensor, low_hw):
    """Must match run_mri_native_batch.map_back."""
    height, width = low_hw
...
def build_parser():
    ...
    parser.add_argument("--expected-sd-revision", required=True)
...
def verify(args):
    ...
    checks = {
        ...
        "checkpoint_sha256": args.expected_vae_sha256,
    ...
    if (manifest.get("sd_snapshot") or {}).get("revision") != args.expected_sd_revision:
        fail("manifest SD snapshot revision mismatch")
    key_report = manifest.get("vae_key_report") or {}
    if key_report.get("missing_keys") != [] or key_report.get("loaded_value_mismatches") != []:
        fail("VAE key report is absent or has missing/mismatched keys")
    ...
    elif sorted(p.name for p in official_dir.iterdir()) != sorted(expected_names):
        fail("official_output does not contain exactly the receipt names")

    pass_count, runtimes, peaks, first_verified = 0, [], [], False
    rng_unchanged = 0  # recorded, not a failure: official order is sorted either way
    for index, (item, row) in enumerate(zip(expected_files, rows)):
        ...
        try:
            ...
            with PIL.Image.open(low_path) as image:
                width, height = image.size
            native_hw = [-(-height // 8) * 8, -(-width // 8) * 8]
            resized = native_hw != [height, width]
            ...
            if row.get("dtype") != EXPECTED_DTYPE or row.get("official_resize_back") is not resized:
                fail(f"{name}: recorded dtype/resize flag mismatch")
            if row.get("official_uint8_match") is not True:
                fail(f"{name}: runner provenance flag not true")
            if row.get("global_rng_unchanged") is True:
                rng_unchanged += 1
            ...
            tensor = None
            if not output_path.exists() or sha256_file(output_path) != row.get("output_file_sha256"):
                fail(f"{name}: output.pt.gz missing or output_file_sha256 mismatch")
            else:
                tensor = load_gz(output_path)
                if list(tensor.shape) != [1, 3, height, width] or str(tensor.dtype) != EXPECTED_DTYPE:
                    fail(f"{name}: tensor shape/dtype mismatch {list(tensor.shape)} {tensor.dtype}")
                    tensor = None
                    ...
                    if tensor_sha != row.get("output_tensor_sha256"):
                        fail(f"{name}: output_tensor_sha256 mismatch")
                    if index == 0 and args.first_row_tensor_sha256:
                        if tensor_sha == args.first_row_tensor_sha256:
                            first_verified = True
                        else:
                            fail(f"{name}: frozen smoke tensor sha256 mismatch ({tensor_sha})")

            native = tensor
            if resized:
                native_path = out_dir / "native_output.pt.gz"
                native = None
                if not native_path.exists() or sha256_file(native_path) != row.get("native_file_sha256"):
                    fail(f"{name}: native_output.pt.gz missing or hash mismatch")
                else:
                    native = load_gz(native_path)
                    if list(native.shape) != [1, 3, *native_hw] or not torch.isfinite(native).all():
                        fail(f"{name}: native tensor shape/finite mismatch")
                        native = None
                    elif thash(native) != row.get("native_tensor_sha256"):
                        fail(f"{name}: native_tensor_sha256 mismatch")
                    elif tensor is not None:
                        diff = np.abs(map_back(native, (height, width))
                                      - tensor[0].numpy().transpose(1, 2, 0)).max()
                        if diff > MAP_BACK_TOLERANCE:
                            fail(f"{name}: map-back recomputation differs (max abs {diff})")

            png_path = args.outputs / str(row.get("official_uint8_png", ""))
            if not png_path.is_file() or sha256_file(png_path) != row.get("official_uint8_png_sha256"):
                fail(f"{name}: official uint8 PNG missing or hash mismatch")
            else:
                official = cv2.imread(str(png_path), cv2.IMREAD_UNCHANGED)
                if official is None or official.shape != (*native_hw, 3) or official.dtype != np.uint8:
                    fail(f"{name}: official uint8 PNG geometry mismatch")
                else:
                    if hashlib.sha256(np.ascontiguousarray(official).tobytes()).hexdigest() != row.get(
                            "official_native_uint8_bgr_sha256"):
                        fail(f"{name}: official uint8 array hash mismatch")
                    if native is not None:
                        bad = bin_violations(native, official)
                        if bad:
                            fail(f"{name}: {bad} float values outside the official uint8 truncation bin")
            official_file = args.outputs / str(row.get("official_output_file", ""))
            if not official_file.is_file() or sha256_file(official_file) != row.get("official_output_file_sha256"):
                fail(f"{name}: official output file missing or hash mismatch")
    ...
    ok = not failures
    return ok, {
        "classification": "MR_ILLUMINATE_OUTPUTS_VERIFIED" if ok else "MR_ILLUMINATE_OUTPUTS_FAILED",
        ...
        "rows_with_global_rng_unchanged": rng_unchanged,
