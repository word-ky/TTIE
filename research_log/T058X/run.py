...
prior_root=Path('/home/wenchang/asdasdsad/wjq/TTIE/runs/20260917-035958-ttie-t058w-native')
prior_files=json.loads(Path('research_log/T058W_archives.json').read_bytes())['files']
for n,h in prior_files.items():assert sha(prior_root/n)==h
rows=[dict(**x,status='not_run',d_rev=None,d_fwd=None,error=None,tolerance=None,ladder=None,boundaries=None,E_decomp=None,d_rev_decomp=None,E_fwd_decomp=None,criteria=None) for x in selection]
backend_initial=torch.backends.mha.get_fastpath_enabled();assert backend_initial is True
error=None;model_hash_receipts=[];cached=[]
for row in rows:
    ...
    row['backend_before']=torch.backends.mha.get_fastpath_enabled()
    try:
        torch.backends.mha.set_fastpath_enabled(False);row['backend_during']=torch.backends.mha.get_fastpath_enabled()
        decomposed=fn(model.v);gd,=torch.autograd.grad(decomposed,model.v)
        de=float(decomposed.detach());dr=float((gd.double()*direction.double()).sum())
        assert torch.isfinite(decomposed) and torch.isfinite(gd).all()
        row.update(E_decomp=de,d_rev_decomp=dr,E_decomp_ulp=ulp(de),criteria=dict(default_decomp_primal=compare(float(value.detach()),de,'primal'),default_decomp_reverse=compare(rev,dr,'derivative')))
        if not all(c['passed'] for c in row['criteria'].values()):
            error='DEFAULT_DECOMPOSED_EQUIVALENCE_FAILURE';row['status']='backend_equivalence_failed'
        else:
            with torch.autograd.forward_ad.dual_level():
                dual=torch.autograd.forward_ad.make_dual(model.v.detach(),direction)
                primal,jvp=torch.autograd.forward_ad.unpack_dual(fn(dual))
                if jvp is None:raise RuntimeError('Native forward AD returned no tangent')
            fw=float(jvp.detach());ep=float(primal.detach());assert torch.isfinite(primal) and torch.isfinite(jvp)
            row.update(d_fwd=fw,E_fwd_decomp=ep,E_fwd_decomp_ulp=ulp(ep),error=abs(dr-fw))
            row['criteria'].update(decomp_forward_primal=compare(de,ep,'primal'),decomp_reverse_forward=compare(dr,fw,'derivative'))
            row['status']='AD_agree' if all(c['passed'] for c in row['criteria'].values()) else 'triangulation_failed'
            if row['status']!='AD_agree':error='TRIANGULATION_CRITERION_FAILURE'
    except Exception as exc:
        error=type(exc).__name__+': '+str(exc);row.update(status='decomposed_execution_unavailable',forward_error=error);(a.out/'forward_error.txt').write_text(traceback.format_exc())
    finally:
        torch.backends.mha.set_fastpath_enabled(row['backend_before']);row['backend_after']=torch.backends.mha.get_fastpath_enabled()
        assert row['backend_after']==backend_initial
    cached.append((row,model,obj,head,direction,before))
...
if error is None and all(r['status']=='AD_agree' for r in rows):
    for row,model,obj,head,direction,before in cached:
        def fn(v):return energy(model,obj,head,v)[0]
        row['ladder']=[]
        for h in STEPS:
            ...
            row['ladder'].append(dict(h=h,plus=plus,minus=minus,central=gap/(2*h),abs_error_vs_rev=abs(gap/(2*h)-row['d_rev']),plus_ulp=up,minus_ulp=um,gap=gap,gap_in_plus_ulps=gap/up,gap_in_minus_ulps=gap/um,plus_clamp=crossings(model,h*direction),minus_clamp=crossings(model,-h*direction)))
        assert before=={n:thash(v) for n,v in model.state_dict().items()}
    write(a.out/'states.json',rows)
for n,h in prior_files.items():assert sha(prior_root/n)==h
...
verdict='T058 directional derivative numerically triangulated' if error is None and all(r['status']=='AD_agree' for r in rows) else 'T058 derivative verifier unresolved'
write(a.out/'receipt.json',dict(classification=verdict,completed_utc=utc(),attempted=sum(r['status']!='not_run' for r in rows),required=16,error=error,model_hashes_before=initial,model_hashes_after=after_models,state_hashes=model_hash_receipts,historical_files_unchanged=historical['files'],prior_verifier_files_unchanged=prior_files,backend_flag_initial=backend_initial,backend_flag_final=torch.backends.mha.get_fastpath_enabled(),source_bindings=binding,source_jpg_opens=0,stage_b_executions=0,target_domain_access=0,official_test_access=0,optimizer_updates=0,persistent_state_changes=0,scientific_source_changes=0,selection_sha256=sha(a.out/'selection.json'),states_sha256=sha(a.out/'states.json'),seconds=time.perf_counter()-start,torch=torch.__version__,cuda=torch.version.cuda,gpu=torch.cuda.get_device_name()))
