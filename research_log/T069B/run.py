...
from research_log.T069B.core import score,threshold,diagnose,weighted_gradients,TOTAL_ATOL,TOTAL_RTOL
HERE=Path('research_log/T069B');MODEL=Path('research_log/T066A/evidence/model.json')
...
def gradients(low,trace,k,expected):
    ...
    parts=losses(x,y);gs=weighted_gradients(parts,renderer.raw)
    direct=torch.autograd.grad(parts @ parts.new_tensor([1.,10.,5.]),renderer.raw)[0].detach().cpu().reshape(-1)
    vectors=torch.stack(gs).detach().cpu().reshape(3,12)
    stats=score(vectors.double().numpy());dg=direct.double().numpy()
    diff=np.abs(np.asarray(stats['summed_gradient'])-dg)
    assert np.all(diff<=TOTAL_ATOL+TOTAL_RTOL*np.abs(dg)),diff
    return dict(**stats,gradient_hashes=[thash(g) for g in vectors],direct_total_gradient=dg.tolist(),direct_total_gradient_hash=thash(direct),direct_total_gradient_norm=float(np.linalg.norm(dg)),total_consistency_max_error=float(diff.max()))
...
def main(out):
    ...
    out.mkdir(parents=True,exist_ok=False);start=time.perf_counter();write(out/'config.json',dict(task='T069-B',source_commit=os.environ['TTIE_SOURCE_COMMIT'],source_binding=binding,started_utc=utc(),optimizer_runs=0,model_fits=0))
    ...
    result=dict(**diagnose(joined),T99_cancel=t99,development_above_T99_cancel=sum(r['R_cancel']>t99 for r in dev),unsafe_rows=[r for r in joined if r['unsafe']],optimizer_runs=0,model_fits=0,completed_utc=utc(),seconds=time.perf_counter()-start)
