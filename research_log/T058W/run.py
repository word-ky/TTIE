...
prior_root=Path('/home/wenchang/asdasdsad/wjq/TTIE/runs/20260917-031943-ttie-t058v-ad')
prior_files=json.loads(Path('research_log/T058V_archives.json').read_bytes())['files']
...
for row in rows:
    ...
    try:
        # Genuine native dual-tensor forward AD on the unchanged graph; no fallback.
        ...
        row.update(d_fwd=fwd,error=abs(rev-fwd),forward_primal=float(primal.detach()),forward_primal_abs_error=abs(float(primal.detach())-float(value.detach())),forward_primal_ulp=ulp(float(primal.detach())),primal_difference_ulps=abs(float(primal.detach())-float(value.detach()))/ulp(float(value.detach())),status='AD_agree' if agreement(rev,fwd) else 'AD_disagree')
