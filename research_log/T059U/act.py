...
source=json.loads(Path('research_log/T059U/source_binding.json').read_bytes())
...
banks=json.loads((B/'training_manifest.json').read_bytes());split=nested_split(banks);selected=split['row_indices']['outer'];wanted=set(selected)
saved_split=json.loads((E/'split.json').read_bytes());assert saved_split['row_indices']==split['row_indices']
auth=json.loads(Path('research_log/T059U/authorization.json').read_bytes());assert sha(C2)==auth['C2_split_sha256'];c2=json.loads(C2.read_bytes())
for key in ['row_indices','bank_indices','image_ids']:assert split[key]['outer']==c2[key]['heldout']
atomic_json(out/'split.json',dict(C2_sha256=sha(C2),image_ids=c2['image_ids']['heldout'],bank_ids=c2['bank_indices']['heldout'],row_ids=selected,zero_overlap_with_E_development=not(set(selected)&set(split['row_indices']['train']+split['row_indices']['heldout']))))
records=[split['canonical'][i] for i in selected];anchors=[(i,r) for i,r in enumerate(records) if r['state_index']==0]
assert len(selected)==1460 and len(anchors)==80 and len({r['bank_index'] for _,r in anchors})==80 and len({r['image_id'] for _,r in anchors})==16
files={str(E/'head.pt'):HEAD,str(E/'split.json'):sha(E/'split.json'),str(B/'training_manifest.json'):sha(B/'training_manifest.json')};features=[];files[str(C2)]=sha(C2)
for bi in split['bank_indices']['outer']:
...
for pos,r in anchors:
    ...
    norm=float(g[pos].double().norm());acted=gate(g[pos]);vg=model.v.detach().cpu().clone() if acted else torch.zeros_like(model.v.detach().cpu());yg=y1.cpu() if acted else y0.cpu()
    tensors=dict(q=q[pos].detach(),g=g[pos].detach(),v1=model.v.detach().cpu(),c1=c1.cpu(),y0=y0.cpu(),y1=y1.cpu(),mask=model.mask.cpu(),detail=model.detail.cpu(),v_gated=vg,y_gated=yg)
    ...
    actions.append(dict(**r,position=pos,norm=norm,acted=acted,tau=TAU,file=name,sha256=sha(out/name),tensor_hashes={k:thash(v) for k,v in tensors.items()},active_regions=int(active.sum()),state_before=before,detail_action_steps=1,optimizer_defaults=optimizer.defaults,persisted_utc=utc()))
atomic_json(out/'actions.json',actions)
for n,h in files.items():assert sha(n)==h,n
for n,h in source.items():assert sha(n)==h,n
atomic_json(out/'action_freeze.json',dict(utc=utc(),actions=80,detail_action_steps=80,head_sha256=HEAD,files={n:sha(out/n) for n in ['field.pt','actions.json','split.json']+[r['file'] for r in actions]},input_hashes_before=files,input_hashes_after=files,jacobian_ranges=reads,source_bindings=source,clean_or_reference_reads_before_action_freeze=0,device=torch.cuda.get_device_name(),gradient_device='CPU, same accepted E CPU gradient convention on fixed1460-row outer batch',tau=TAU,gated_acted=sum(r['acted'] for r in actions),gated_action_reuses_identical_ungated_displacement=True,evaluation='Absolute RGB MSE change, all80 anchors; no reference eligibility',normalization=head.normalization()))
