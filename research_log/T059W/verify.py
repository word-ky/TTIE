...
def main(out):
    ...
    records=json.loads((out/'online_records.json').read_bytes());table=json.loads((out/'comparison_table.json').read_bytes());result=json.loads((out/'result.json').read_bytes());selection=json.loads(Path('research_log/T059W/selection.json').read_bytes());assert [r['bank_index'] for r in records]==[r['bank_index'] for r in selection['anchors']]
    raw_actions=json.loads((U/'actions.json').read_bytes())
    for selected in selection['anchors']:
        candidates=[a for a in raw_actions if a['image_id']==selected['image_id'] and a['state_index']==0]
        maximum=max(a['norm'] for a in candidates);chosen=sorted(a['bank_index'] for a in candidates if a['norm']==maximum)[0]
        assert chosen==selected['bank_index'] and maximum==selected['selected_norm'] and maximum>0
        assert next(a['active_regions'] for a in candidates if a['bank_index']==chosen)==selected['selected_active_regions']>0
    assert len(set(a['image_id'] for a in selection['anchors']))==16
    sf=json.loads(Path('research_log/T059W/selection_freeze.json').read_bytes());assert sha('research_log/T059W/selection.json')==sf['sha256'] and sf['utc']<f['utc']
    ...
    assert len(checks)==16 and sum(ok)==result['passed'];assert result['classification']==('active-path online target-free Jacobian bridge is reproducible across the fixed outer source images' if all(ok) else 'active-path online target-free Jacobian bridge is not reproducible')
