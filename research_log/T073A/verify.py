...
def check(registry, plan, historical, manifest):
    ...
    for row in rows:
        assert row["tier"] == (2 if row["id"] == "gm_moe" else 1)
    ...
    assert "all Tier-1 rows" in plan["reference_gate"]
    assert "ZERO-IG and GM-MoE inclusion locked" in plan["reference_gate"]
