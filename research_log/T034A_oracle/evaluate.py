...
for k in ['split','normal-root','prior-oracle','preflight','out']:p.add_argument('--'+k,type=Path,required=True)
...
for i,(r,baseline,binding) in enumerate(zip(split,old,f['rows'])):
    assert r['low']==baseline['low']==binding['low'];d=a.out/f'{i:03d}'
    ...
    for j,h in enumerate(hist):
        assert h['raw'].shape==(501,1,5,2,2) and h['mse'].shape==(501,) and state['starts'][j]['updates']==500
        assert torch.isfinite(h['raw']).all() and torch.isfinite(h['mse']).all()
        raw=h['raw'][:,0];grid=physical_parameters(torch.cat([raw,torch.zeros(501,1,2,2)],1))
    ...
    for ch,name in enumerate(['ev','gamma','wb_r','wb_g','wb_b']):
        for y in range(2):
            for x in range(2):
...
metrics={k:stats([r[k] for r in rows]) for k in ['identity_psnr','identity_ssim','selected_psnr','selected_ssim','oracle_psnr','oracle_ssim','t028_psnr','t028_ssim','delta_psnr','delta_ssim']}
mean=metrics['delta_psnr']['mean'];median=metrics['delta_psnr']['median']
classification='substantial chromatic action headroom' if mean>=1.5 and median>=1 else ('limited chromatic action headroom' if mean<.5 else 'mixed chromatic action headroom')
distributions={}
for name in ['ev','gamma','wb_r','wb_g','wb_b']:
    distributions[name]={}
...
o.write(a.out/'evaluation_receipt.json',dict(label=o.LABEL,completed_utc=o.utc(),freeze_sha256=o.sha(a.out/'freeze.json'),all200_histories_finite_bounded=True,all_earliest_winners_verified=True,
    independent_metric_max_abs_error=error,opened_normals=opened,prior_csv_sha256=o.sha(a.prior_oracle/'per_image.csv'),official_test_access=False))
