...
The deployable bottleneck therefore remains two-sided: the action family must be expressive enough, and the learned reference-free optimization field must remain valid over the states it visits.

## Capacity sequence through T051
...
- **T051-A smooth RGB-shared 8×8 exposure field:** **`22.5332679 / 0.5219925`**. Relative to T050, paired PSNR is **`+0.6590052 dB` mean / `+0.4525454 dB` median** and mean RGB-SSIM is `+0.0168318`; PSNR improves on `100/100`, SSIM on `96/100`. It misses the frozen `+1.50/+0.75 dB` promotion gate, so multiplicative spatial exposure alone is **not supported as a SOTA-scale capacity jump**. Nevertheless the effect is real and broad. `23/100` winners are at step 500 and the selected exposure field has no exact ±2 EV hits, so simple budget/range saturation is not the leading explanation.

T051 changes the capacity story: intra-region spatial illumination is useful, but the scale-only field still leaves the accepted reference reachability at `22.5333 dB`, about `0.8631 dB` below the SNR-Aware training-exposed development PSNR anchor and far below the eventual `+2–3 dB` sprint objective. The next high-value capacity question is whether the regional T048 scale/offset interaction reappears at the smooth spatial-field level.
...
- Fresh/final benchmark sets must remain isolated from model/hyperparameter selection. **The official LOL-v2 Real test remains untouched through T051-A.**
...
- **Best accepted non-deployable reference reachability:** T051-A, **`22.5332679 / 0.5219925`**.
...
T001–T013: controlled mechanism/diagnostic sequence. T014: broad controlled/fresh Sobolev Ours-Core. T019: heterogeneous adaptive geometry positive. T020: universal geometry route paused. T021: RGB-SSIM transfer positive. T024: baseline protocol frozen. T026-A: fixed-validation deployable base. T027-A/B: baseline exporters ready. T029: late-field mismatch. T031/T032: support-distance association positive but controller negative. T033: Retinexformer development anchor. T036: common-gain fresh target-free aggregate positive with unsafe tail. T041/T042: late real field-reliability failure and early-state directional rescue. T043: directional validity versus absolute quality separated. T044: simple excursion risk signal rejected. T045: SNR-Aware anchor `23.3963 / 0.8238`. T046: common-gain budget extension negative. T047: lift broad-positive/sub-gate. T048: regional affine coupling positive. T049: nonlinear monotonic tone positive. T050: tone underconvergence rejected. **T051: smooth spatial exposure is broad-positive (`+0.6590 dB` mean, `+0.4525 dB` median; 100/100 PSNR wins) but below the SOTA-scale promotion gate.**

PR #75/T050 is merged as `276c0b1fa5c9e6548bef90048ec6fcc43da439c3`. PR #76/T051 is scientifically accepted; after the dependency was squash-merged it is currently non-mergeable because of Git history/content overlap, which is an integration issue rather than a scientific rejection. Preserve its exact accepted head/evidence state.

## Current open task

**T052-A — smooth spatial affine-coupling closure oracle** in `coordination/CHATGPT_TO_CODEX.md`.

This is one isolated `REFERENCE_ORACLE_ONLY` probe starting from exact accepted T051 states. It jointly re-optimizes only the accepted smooth exposure field plus one new smooth RGB-shared additive field while every pre-T051 coordinate remains frozen. No sweep, retraining, fresh cohort, baseline rerun, deployable use of references, or official-test access is authorized.