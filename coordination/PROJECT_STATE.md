...
The matched-detail program has established a strong asymmetry: **detail-direction information transfers much better than calibrated scalar energy/value, but direct finite-step use of that direction is not yet supported.**
...
- **T059-T:** the frozen first-order/finite-step decomposition rejects the proposed Adam-scale/curvature explanation. `57/61 = 0.9344262` eligible anchors have `L=<g_R,Δv><0`, but **0/4** actual harmful anchors are `L<0,A>0` overshoot flips; all four harmful steps already have the wrong first-order sign along the persisted action. Median Adam saturation is `1.0`, so coordinatewise magnitude-normalization is present but does not explain these harms. The direct optimizer-scale rescue is therefore closed. An exploratory target-free clue remains: all four harmful anchors have predicted-gradient norm below the eligible-anchor p10 `0.031453661388567547`; this is hypothesis generation only until checked on a separate source cohort.

### Current scientific interpretation after T059-T
...
The detail branch is also narrower now. T059-S showed that transferred direction can produce many tiny finite improvements but is not safe under the preregistered one-step gate. T059-T then showed that the observed harms are **not** cases where a locally descending step merely overshoots because of Adam scaling; their first-order sign is already wrong. Thus magnitude-preserving optimizer changes, learning-rate/epsilon sweeps, and multi-step extensions are not justified. The only remaining mechanism candidate supported strongly enough for one more source-only check is a **target-free confidence/abstention rule based on predicted detail-gradient magnitude**: every T059-T harmful anchor lies below the inner-held p10 norm. Because that pattern was discovered on the opened inner-held cohort, it cannot be validated there; T059-U tests one frozen threshold on the separate T059-C2 outer source cohort with all actions frozen before any outer reference is read.

The matched-detail line remains **non-deployable**. No multi-step detail TTT, target-domain detail action, LOL-v2 detail action, official-test detail action, or real-domain detail rollout is authorized.
...
- Fresh/final benchmark sets must remain isolated from model/hyperparameter selection. **The official LOL-v2 Real test remains untouched through T059-T.**
...
- **Matched-detail candidate:** scalar calibration/localization, simple dual-head uncertainty, direct inherited-Adam one-step integration, and the specific optimizer-scale/curvature rescue are not supported. The only active question is whether a single source-derived, target-free low-predicted-gradient-norm abstention rule transfers to a separate source outer cohort; T059-U is validation-only and remains non-deployable.
- **Training-exposed development anchors:** Retinexformer `21.4787864 / 0.7900612`; SNR-Aware `23.3963299 / 0.8237644`.

## Integration note

Historical PRs may contain inherited history/integration complications. Preserve accepted scientific/evidence states rather than repairing history inside experiment cycles. T059-P is PR #115, T059-Q is PR #116, T059-R is PR #117, T059-S is PR #118, and T059-T is PR #119; they are scientifically reviewable through their pinned source/evidence even while PRs remain open.

## Current open task

**T059-U — fixed low-gradient-norm abstention on the T059-C2 outer source cohort** in `coordination/CHATGPT_TO_CODEX.md`.

Using the accepted T059-E head and exactly the separate 16-image/80-bank T059-C2 outer source cohort, compute the T059-S state-0 predicted local-detail gradient and apply one immutable global rule: abstain when `||g_hat||_2 < 0.031453661388567547`, otherwise take the exact same single fresh Adam step. All gradient norms, gate decisions, actions, and outputs must be frozen before any outer clean/reference read. The task tests transfer of one target-free confidence hypothesis only; there is no threshold search, retraining, second step, target-domain access, LOL-v2 access, official-test access, or real-domain rollout.
