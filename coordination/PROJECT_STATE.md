...
## Capacity sequence through T052
...
- **T052-A joint smooth exposure + additive field:** **`23.1071511 / 0.5700402`** (median PSNR `23.8982378`). Relative to T050, paired PSNR is **`+1.2328884 dB` mean / `+1.0713880 dB` median** and mean RGB-SSIM is **`+0.0648795`**. The sole frozen gate is formally **negative** because the mean-PSNR requirement was `+1.50 dB`; median and SSIM conditions pass. Relative to T051, T052 adds **`+0.5738832 dB` mean / `+0.5183313 dB` median PSNR** and `+0.0480477` mean SSIM. PSNR improves on 100/100 images versus both T050 and T051; SSIM improves on 99/100 versus T051. Because T052 continued `u` while adding `b`, this supports the joint spatial-affine family broadly but does not isolate scale/offset interaction from continued exposure optimization.

The T052 verifier-only coordinate-rounding repair is accepted as an evidence correction: scientific source/trajectory/metrics/tolerances are unchanged, and corrected independent replay passes all 50,100 states / 200 selected metrics / 1,132 scalar checks with max renderer/interpolation errors below `1e-6`.

T052 changes the capacity picture materially. The best accepted compact reference reachability is now **23.1072 dB**, only about **0.2892 dB below** the SNR-Aware training-exposed development PSNR anchor, although RGB-SSIM remains about **0.2537 lower**. The most specific remaining confound is the additive-field bound: `1541/6400 = 24.08%` of selected `b` controls sit exactly at `+0.20` and 82 at `-0.20`, while selected exposure controls have no exact ±2 EV hits. `32/100` T052 winners are at step 500. This motivates one fixed additive-range closure probe, not a generic budget/LR sweep.
...
- Fresh/final benchmark sets must remain isolated from model/hyperparameter selection. **The official LOL-v2 Real test remains untouched through T052-A.**
...
T001–T013: controlled mechanism/diagnostic sequence. T014: broad controlled/fresh Sobolev Ours-Core. T019: heterogeneous adaptive geometry positive. T020: universal geometry route paused. T021: RGB-SSIM transfer positive. T024: baseline protocol frozen. T026-A: fixed-validation deployable base. T027-A/B: baseline exporters ready. T029: late-field mismatch. T031/T032: support-distance association positive but controller negative. T033: Retinexformer development anchor. T036: common-gain fresh target-free aggregate positive with unsafe tail. T041/T042: late real field-reliability failure and early-state directional rescue. T043: directional validity versus absolute quality separated. T044: simple excursion risk signal rejected. T045: SNR-Aware anchor `23.3963 / 0.8238`. T046: common-gain budget extension negative. T047: lift broad-positive/sub-gate. T048: regional affine coupling positive. T049: nonlinear monotonic tone positive. T050: tone underconvergence rejected. T051: smooth spatial exposure broad-positive but below promotion gate. **T052: joint smooth spatial affine family reaches `23.1072 / 0.5700`, broad-positive on all 100 PSNR cases but formally misses the predeclared total mean-PSNR gate; additive control saturation becomes the next specific capacity question.**

PR #75/T050 is merged as `276c0b1fa5c9e6548bef90048ec6fcc43da439c3`. PR #76/T051 and PR #77/T052 are scientifically accepted but currently non-mergeable because their evidence branches retain pre-squash dependency history; this is an integration issue, not a scientific rejection. Preserve the exact accepted heads/evidence states rather than rewriting history during experiment cycles.

## Current open task

**T053-A — fixed additive-range closure oracle** in `coordination/CHATGPT_TO_CODEX.md`.

This is one isolated `REFERENCE_ORACLE_ONLY` probe starting from exact accepted T052 selected states. It freezes the accepted exposure field and every older coordinate, trains only the existing smooth additive `b` field, and changes exactly one constraint: `b` projection from `[-0.20,+0.20]` to `[-0.40,+0.40]`. No range sweep, extra exposure optimization, retraining, baseline rerun, fresh cohort, deployable use of references, or official-test access is authorized.
