...
The deployable bottleneck remains two-sided: the renderer/action family must be expressive enough, and the learned target-free optimization field must remain valid over unseen images and the states/directions it visits. T054–T057 establish that local detail/noise control is the strongest missing renderer capability found so far. T058-AF shows the frozen T014 field does not transfer strongly enough to that 64-D detail tangent; T059-BF shows the same 28-D head can fit legacy and detail tangents jointly in-source; T059-C2 then shows that this fixed fit does not satisfy the preregistered unseen-image gate. The current scientific question is therefore **why cross-image transfer fails**—especially whether the large scalar-value error is merely additive bank-wise calibration while relative energy geometry remains useful—before any new training recipe or real-domain rollout is considered.
...
- Fresh/final benchmark sets must remain isolated from model/hyperparameter selection. **The official LOL-v2 Real test remains untouched through T059-C2.**
...
- **Matched-detail source-fit candidate:** T059-BF/T059-C2 dual-tangent line. In-source fitting succeeds, but the fixed image-held-out generalization gate fails; this line is **not deployable** and cannot proceed to real-domain detail rollout without a new scientifically justified generalization fix and fresh validation.
...
PR #75/T050 is merged as `276c0b1fa5c9e6548bef90048ec6fcc43da439c3`. PRs #76–#83 inherit evidence-history/integration complications; preserve exact accepted scientific/evidence states rather than rewriting history during experiment cycles. PR #83/T057 may remain non-mergeable for history reasons; that does not invalidate its reviewed scientific evidence. PR #95/T058-AF, PR #99/T059-BF, and PR #101/T059-C2 are likewise scientifically reviewable despite inherited history; do not repair history inside experiment cycles.

## Current open task

**T059-D — frozen bank-relative value/gradient audit** in `coordination/CHATGPT_TO_CODEX.md`.

Use only the frozen T059-C2 checkpoint and its already-observed held-out source caches; do not train anything. Reproduce the C2 aggregates, then test whether subtracting a deterministic `state_index==0` anchor within each held-out bank brings standardized log-MSE value Huber back inside the existing `0.0765085` ceiling while leaving the original global directional verdict untouched. Also report within-bank ordering/regret and per-bank directional distributions for diagnosis only. This task cannot reverse the T059-C2 negative, cannot tune a replacement on the observed holdout, and does not authorize target-domain TTT, LOL-v2, official-test access, PSNR/SSIM selection, clean/reference information at test time, or real-domain detail rollout.
