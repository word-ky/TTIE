...
## Capacity sequence through T054
...
- **T054-A fixed local-detail field:** **`24.3454867 dB / 0.7577572 RGB-SSIM`** (median PSNR `24.8107619`, median SSIM `0.7766555`). Starting from exact accepted T052 and freezing every old coordinate, one RGB-shared 8×8 coefficient field multiplies the fixed image-derived basis `D=y0-B(y0)` with a deterministic separable 5×5 binomial blur. Relative to T052, T054 adds **`+1.2383356 dB` mean / `+1.2353360 dB` median PSNR and `+0.1877170` mean RGB-SSIM**, with both metrics improving on **100/100** images. Independent replay passes 50,100 states / 200 metrics / 720 scalar checks with basis/interpolation/renderer errors below `1e-6`. The selected coefficient field is strongly negative (full-field mean `-0.7859`, median `-0.9392`), showing that spatially varying detail attenuation / denoising is the dominant newly exposed degree of freedom. **All 100/100 winners are at step 500**, so T054 is not yet a convergence certificate.

The capacity diagnosis has therefore changed materially. Local-frequency/detail control is a major missing renderer capability. The best accepted compact reference reachability is now **T054: `24.3455 / 0.7578`**, not T052. This exceeds the training-exposed SNR-Aware development PSNR anchor by about `0.95 dB`, but still trails its RGB-SSIM by about `0.066`; neither comparison is held-out evidence. Because every T054 winner is at the optimization boundary and coefficients already concentrate near the negative tanh asymptote, the next isolated question is fixed-budget convergence of the same one-scale detail family before any range/multiscale redesign.
...
- Fresh/final benchmark sets must remain isolated from model/hyperparameter selection. **The official LOL-v2 Real test remains untouched through T054-A.**
...
- **Best accepted non-deployable reference reachability:** T054-A, **`24.3454867 / 0.7577572`**.
...
T001–T013: controlled mechanism/diagnostic sequence. T014: broad controlled/fresh Sobolev Ours-Core. T019: heterogeneous adaptive geometry positive. T020: universal geometry route paused. T021: RGB-SSIM transfer positive. T024: baseline protocol frozen. T026-A: fixed-validation deployable base. T027-A/B: baseline exporters ready. T029: late-field mismatch. T031/T032: support-distance association positive but controller negative. T033: Retinexformer development anchor. T036: common-gain fresh target-free aggregate positive with unsafe tail. T041/T042: late real field-reliability failure and early-state directional rescue. T043: directional validity versus absolute quality separated. T044: simple excursion risk signal rejected. T045: SNR-Aware anchor `23.3963 / 0.8238`. T046: common-gain budget extension negative. T047: lift broad-positive/sub-gate. T048: regional affine coupling positive. T049: nonlinear monotonic tone positive. T050: tone underconvergence rejected. T051: smooth spatial exposure broad-positive but below promotion gate. T052: joint smooth spatial affine family reaches `23.1072 / 0.5700`. T053: additive-range rescue rejected. **T054: a single compact spatial local-detail field produces a large universal `+1.238 dB / +0.1877 SSIM` gain and raises reference reachability to `24.3455 / 0.7578`, establishing local-frequency/detail control as a major missing capacity.**

PR #75/T050 is merged as `276c0b1fa5c9e6548bef90048ec6fcc43da439c3`. PR #76/T051, PR #77/T052, PR #78/T053, and PR #79/T054 inherit non-mergeable evidence history; preserve exact accepted scientific/evidence states rather than rewriting history during experiment cycles.

## Current open task

**T055-A — fixed local-detail convergence extension** in `coordination/CHATGPT_TO_CODEX.md`.

Start from exact accepted T054 selected `v`, keep the entire renderer and coefficient range unchanged, and continue only `v` for a fixed +1000 fresh-Adam updates at the same `lr=0.05`. This is one isolated `REFERENCE_ORACLE_ONLY` convergence test. No range/LR/grid/kernel/scale sweep, second start, additional basis, joint reoptimization, retraining, baseline rerun, fresh cohort, deployable use of references, official-test access, or T056 is authorized.
