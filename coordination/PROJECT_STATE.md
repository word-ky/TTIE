...
- **T047-A:** a bounded RGB-shared additive lift after common gain is broadly helpful but **not materially sufficient under the frozen marginal-capacity gate**. With every T046 EV/gamma/common-gain coordinate frozen, mean PSNR rises from `19.5732278418` to `20.0559720402 dB` and median from `18.9220694331` to `19.2949802533`, paired `+0.4827441984 / +0.1331822579 dB`, missing the predeclared `+0.50 / +0.25 dB` gate. Mean RGB-SSIM rises `+0.0049064412`; PSNR improves on `100/100` and SSIM on `81/100`. No selected state is at step 500 and only `12/392` active lift coordinates hit `+0.20`, so simple T047 budget saturation is not the explanation.

T046/T047 narrow the capacity story. Simply running the EV/gamma/common-gain family longer does not recover the large development gap, while an offset direction produces a consistent but typically small gain. T047 therefore does **not** justify adding lift to deployable TTT yet, but it also should not be summarized as a null operator. The remaining cheap question is whether the post-gamma scale and offset coordinates need to be optimized jointly; this is tested once in T048 before introducing a genuinely new nonlinear image-formation coordinate or investing in field retraining.
...
9. T047 shows a post-gamma offset direction is consistently useful but too small in typical per-image magnitude to pass the material marginal-capacity gate when gain is frozen.
10. The immediate capacity question is **post-gamma affine closure**: does allowing scale and offset to co-adjust add a material increment beyond T047 lift-only? T048 answers that once. If not, the next capacity investment should move to a genuinely nonlinear renderer coordinate rather than another affine reparameterization.
...
- **Additive-lift-on-frozen-common-gain non-deployable reachability:** T047-A, `20.0559720 / 0.4142735`; broad positive but fails the frozen material marginal-capacity gate.
