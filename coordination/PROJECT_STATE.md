...
**Phase B update (2026-09-25 16:45+08):** The above PromptIR OOM paragraph is historical. A prospective operator-equivalent, not bitwise-identical native-4K memory schedule passed synthetic full-model testing and was sealed on GitHub before one target-low retry. That single static PromptIR execution-only smoke passed with finite float32 native output and unchanged 548-key state; report `research_log/T073B/target_static_smoke_report.md`. Static full 150 outputs and DCTTA native smoke/full outputs remain pending, so neither row is frozen. Ours-TTT remains scientifically blocked pending explicit research-lead decision. Target references/metrics remain sealed/zero.
