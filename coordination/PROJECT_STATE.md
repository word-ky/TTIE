...
- **T035-A:** a matched RGB-shared common-gain reference oracle reaches `19.5530229 / 0.4084669`. It explains `85.50%` of T034's mean PSNR gain and `81.82%` of its median gain, showing that the dominant added capacity is a post-gamma common-intensity degree rather than independent chromatic WB. Its original 500-step run had 85/100 winners at the budget boundary, motivating the fixed T046 convergence probe.
...
- **T046-A:** material T035 underconvergence is **not supported** under the predeclared fixed extension. Starting from each exact accepted T035 winner, one fresh Adam optimizer at `lr=0.05` for exactly +1000 reference-MSE updates raises mean PSNR only from `19.5530228894` to `19.5732278418` and median only from `18.8982131190` to `18.9220694331`: paired `+0.0202049524 dB` mean / `+0.0123440643 dB` median, far below the frozen `+1.00 / +0.75 dB` gate. Mean RGB-SSIM improves only `+0.0009001422`. PSNR is 99 win / 1 equal / 0 loss, but the gains are tiny; 49/100 new winners remain at step 1000.

T046 is a key capacity correction. The old T035 endpoint frequency did **not** hide a large optimization-budget gain under this fixed continuation, so simple finite-budget underconvergence is no longer a credible explanation for the remaining development gap. This does not certify a global optimum or hard action-family ceiling: the probe is still single-start/fixed-optimizer and 49/100 winners remain on the new boundary. But it materially strengthens the case that the current EV/gamma/common-gain renderer itself lacks important degrees of freedom. The next question is therefore one complementary coordinate at a time, before any expensive field retraining.
...
13. T034/T035 show that adding common post-gamma intensity captures most of the extra RGB-gain oracle PSNR capacity, but even that reference oracle remains below the strong training-exposed anchors.
14. T046 rejects **material fixed-budget underconvergence** as the dominant explanation for T035: +1000 additional updates yield only `+0.0202 dB` mean / `+0.0123 dB` median PSNR. Endpoint occupancy remains high, so global optimality is not proven, but simply running the same action family longer is no longer justified.
15. The immediate capacity question is now **renderer complementarity**. T047 tests one additive black-level/lift coordinate in isolation; only if it gives material reference-only marginal capacity should we consider how to learn/adapt that coordinate without targets.
...
- **Common-gain non-deployable reference reachability after fixed convergence probe:** T046-A, `19.5732278 / 0.4093671`; only `+0.0202 dB` mean over T035 despite +1000 updates. Not a certified global ceiling.
