...
**T073-A-R1 locally verified; research-lead acceptance pending.** The initial T073-A seal claim was superseded by the research lead's R1 findings. Revised `research_log/T073A/` now fixes exactly eight Tier-1 rows; ZERO-IG is preferred additional and GM-MoE secondary. PromptIR and PromptIR+DCTTA remain `PENDING_SOURCE_BINDING` to one shared official base, to be chosen from source provenance before target execution. DCTTA has `LOW_ONLY_DCTTA_REQUIRED`, requiring a sealed, independently verified low-only wrapper if official paired-loader plumbing touches target references. The original T072-L metric/bootstrap and accepted T072-AZ-B manifest remain unchanged; the independent verifier and 17 rejection tests pass. Only RetinexFormer/SNR-Aware are `FROZEN_OUTPUTS`; eight rows remain `PENDING_OUTPUTS`. `reference_reads=0`, `metrics=0`, `model_runs=0`. No PromptIR/DCTTA or other pending method was run. UHD-LL references remain sealed; await research-lead acceptance before the next execution task.
