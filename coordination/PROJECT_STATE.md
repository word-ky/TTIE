...
## Unsafe-tail diagnosis through T041-A
...
**T040-A — no gain-specific source failure even at the T036 high-gain range.** T040 reuses all 7,346 accepted source states and probes only common gain `1.50` and `1.75`. At gain `1.75`, legacy EV+gamma is `79.6496%` positive-dot with median cosine `0.770274`; gain is `73.0546% / 0.612039`. The same-gain legacy-minus-gain gaps are only `6.5949 pp / 0.158235`, so the frozen `20 pp / 0.25` gate is not met: **source high-gain tangent deficit is not supported**. High gain still degrades alignment for both source coordinate groups relative to gain `1.25`, so range extrapolation is real but shared rather than gain-specific.

**T041-A — strong matched-gain real selected-state deficit.** Holding common gain fixed to `1.75`, the accepted source total-group baseline is `79.6634%` positive-dot with median cosine `0.763459`, whereas all 100 T036 real selected states give only `37% / -0.185848`. The deficits are `42.6634 pp / 0.949307`, decisively passing the frozen `20 pp / 0.25` gate: **real selected-state field deficit beyond source high-gain is supported**.

The T041 group breakdown changes the mechanism interpretation. At fixed gain `1.75`, the real gain group remains `71%` positive-dot with median cosine `0.309086`, while the real legacy EV+gamma group is only `37% / -0.220698`; source at the same gain is `73.05% / 0.612039` for gain and `79.65% / 0.770274` for legacy. Thus the all-100 real collapse is dominated by the legacy/feature-state part of the field, not by an all-image gain-coordinate failure. Gain cosine remains worse than source, so gain is not fully reliable. The same qualitative pattern appears at fixed gain `1.25`.

T041 does **not** prove a pure image-content domain shift because the populations differ in state: source uses canonical source states, while real uses late T036 selected legacy EV+gamma/feature states. Its information boundary is accepted: all 200 low-only probes were frozen before normal/reference access; original selected outputs were bit-exact; Stage B reused only already-reference-used T036 normals and reproduced every probe output exactly; there were zero optimizer/selection/deployable changes and no official-test access; independent replay checked 4,492 scalars with max error `1.95e-13`. T041 is `REFERENCE_GRADIENT_DIAGNOSTIC_ONLY`.
...
7. T039 and T040 reject the simplest source-side explanation: the gain tangent is not intrinsically broken near identity and does not become uniquely broken on source at gain `1.75`.
8. T041 shows that even after gain is matched, the all-100 real selected-state field is dramatically worse than source; importantly, the dominant all-100 deficit is in legacy EV+gamma/feature-state alignment while gain positive-dot remains comparatively intact.
9. Therefore the strongest current hypothesis is no longer “gain is the main unsafe coordinate.” It is a **real late selected-state/feature-distribution failure with a loss-subset-specific gain interaction**. Current evidence still does not separate real image-content shift from late legacy-state/feature extrapolation. T042 is designed to test that next minimal distinction on the same real images.
...
- Fresh/final benchmark sets must remain isolated from model/hyperparameter selection. The official LOL-v2 Real test is still untouched through T041-A.
- Fresh/test runs must fail closed on source/checkpoint/cohort/provenance mismatches.

## Milestones

T001–T013: controlled mechanism/diagnostic sequence. T014: broad controlled/fresh Sobolev Ours-Core. T019: heterogeneous adaptive geometry positive. T020: universal geometry route paused. T021: RGB-SSIM transfer positive. T022/T023: real validation/action-range and tiny recalibration sequence. T024: baseline protocol frozen. T025/T028: reachability diagnosed. T026-A: fixed-validation deployable base. T027-A/B: baseline exporters ready. T029: late-field mismatch. T030: self-reversal guard negative. T031: support-distance association positive. T032: support-radius controller negative. T033: Retinexformer development anchor. T034/T035: RGB-WB capacity resolved mainly to common intensity. T036: common gain fresh target-free aggregate positive with unsafe tail. T037: late-selection headroom limited/mixed. T038: real loss-subset gain mismatch. T039: near-range source gain-tangent deficit not supported. T040: high-range gain-specific source deficit not supported. **T041: matched-gain all-100 real selected-state deficit strongly supported and dominated by legacy/feature-state collapse.**

## Current open task

**T042-A — fixed-gain early-vs-selected legacy-state audit** in `coordination/CHATGPT_TO_CODEX.md`.

Reuse exactly the same 100 already-reference-used T036 real images and accepted trajectories. For each image, replace only the selected legacy EV+gamma coordinates with that image's fixed T036 step-10 legacy coordinates, keep its gate unchanged, and set active common gain to the single fixed value `1.75`. Freeze exactly 100 low-only learned gradients and outputs before opening any reference, then compute isolated RGB-MSE reference gradients on the same already-used normals. Compare the all-100 total alignment against the bound T041 selected-state gain=`1.75` baseline under one predeclared `+20 pp / +0.25` improvement gate. This is diagnostic only: no optimization, stopping-rule qualification, new cohort, deployable change, or official-test access.
