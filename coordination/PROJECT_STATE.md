...
The deployable bottleneck remains two-sided: the renderer/action family must be expressive enough, and the learned target-free optimization field must remain valid over unseen images and the states/directions it visits. T054–T057 establish that local detail/noise control is the strongest missing renderer capability found so far. T058-AF shows the frozen T014 field does not transfer strongly enough to that 64-D detail tangent; T059-BF shows the same 28-D head can fit legacy and detail tangents jointly in-source; T059-C2 then shows that this fixed fit does not satisfy the preregistered unseen-image gate; T059-D shows post-hoc additive bank centering cannot rescue the held-out scalar-value failure; and T059-E shows that learning a state-0-relative amplitude objective directly still does not transfer. Because held-out rank ordering remains strong while relative amplitude error is large, the next justified question is whether the remaining scalar failure is largely a **positive bankwise scale** nuisance versus a genuinely non-monotonic within-bank geometry error. That question must be answered diagnostically before another training redesign.
...
- Fresh/final benchmark sets must remain isolated from model/hyperparameter selection. **The official LOL-v2 Real test remains untouched through T059-E.**
...
- **Matched-detail source-fit candidate:** T059-BF/T059-C2/T059-E line. In-source fitting succeeds, but both the fixed image-held-out absolute-value recipe and the later bank-relative nested recipe fail their transfer gates. T059-D rejects additive-offset-only rescue, and T059-E shows relative-amplitude training is still insufficient. This line is **not deployable** and cannot proceed to real-domain detail rollout without a new scientifically justified generalization fix and fresh validation.
...
PR #75/T050 is merged as `276c0b1fa5c9e6548bef90048ec6fcc43da439c3`. PRs #76–#83 inherit evidence-history/integration complications; preserve exact accepted scientific/evidence states rather than rewriting history during experiment cycles. PR #83/T057 may remain non-mergeable for history reasons; that does not invalidate its reviewed scientific evidence. PR #95/T058-AF, PR #99/T059-BF, PR #101/T059-C2, PR #102/T059-D, and PR #103/T059-E are likewise scientifically reviewable despite inherited history; do not repair history inside experiment cycles.

## Current open task

**T059-F — frozen positive-scale decomposition of the T059-E held-out value failure** in `coordination/CHATGPT_TO_CODEX.md`.

Use only the already-observed T059-E inner-held persisted evidence and exact frozen checkpoint; do not read the excluded C2 outer supervision. Replay T059-E exactly, then after the fixed state-0 offset fit only the preregistered closed-form nonnegative per-bank diagnostic scale and recompute row-weighted relative Huber. This is a no-training decomposition to determine whether the remaining scalar failure is predominantly positive-scale calibration or genuine within-bank shape error. It cannot reverse T059-E, calibrate deployment, authorize target-domain TTT, access LOL-v2/official test, use clean/reference information at test time, or trigger real-domain detail rollout.