...
- **T049-A:** an RGB-shared 8-segment monotonic regional tone LUT is **useful but does not pass the frozen multi-dB gate at 500 steps**. With all T048 EV/gamma/gain/lift coordinates frozen, reference-only mean PSNR/RGB-SSIM rises from `21.0649799860 / 0.4566638129` to **`21.8722552498 / 0.5047578705`**. Paired PSNR is **`+0.8072752637 dB` mean / `+0.5988926101 dB` median**; mean SSIM is `+0.0480940576`; PSNR improves on `100/100`. The predeclared `+2.00/+1.00 dB` SOTA-scale gate fails. Crucially, `91/100` winners are exactly at step 500, so unlike T046 the fixed-budget result is strongly convergence-confounded and is not evidence that monotonic-tone capacity is exhausted.

T046–T049 sharpen the capacity story. The original common-gain family is not rescued by more steps; affine scale/offset coupling adds a real `+1.0 dB`; and nonlinear monotonic tone adds another broad `+0.81 dB` mean with a very large SSIM increment. T049 already exceeds the Retinexformer training-exposed PSNR anchor by `0.39347 dB` in this reference-only diagnostic, but remains `1.52407 dB` below SNR-Aware in PSNR and far below both anchors in SSIM. Because `91/100` T049 winners hit the fixed boundary, one frozen convergence extension is required before abandoning the tone coordinate or moving to a richer spatial residual/illumination state.
...
11. T049 proves a compact nonlinear monotonic tone coordinate adds broad, nontrivial capacity and substantial SSIM, but its 500-step multi-dB verdict is unresolved because `91/100` winners are boundary-selected. T050 tests that convergence confound once; after T050, do not continue pure budget extensions.
...
- **Monotonic-tone-on-frozen-affine non-deployable reachability:** T049-A, **`21.8722552 / 0.5047579`**; useful and universal in PSNR, but strongly boundary-limited at 500 steps and not a certified ceiling.
