...
The deployable bottleneck remains two-sided: the renderer/action family must be expressive enough, and the learned target-free optimization field must remain valid over unseen images and the states/directions it visits. T054–T057 establish that local detail/noise control is the strongest missing renderer capability found so far. T058-AF shows the frozen T014 field does not transfer strongly enough to that 64-D detail tangent; T059-BF shows the same 28-D head can fit legacy and detail tangents jointly in-source; T059-C2 then shows that this fixed fit does not satisfy the preregistered unseen-image gate; T059-D further shows that post-hoc additive bank centering cannot rescue the held-out scalar-value failure. Because the deployable TTT gradient and within-trajectory argmin are invariant to a shared additive bank constant, the current scientifically justified training hypothesis is to learn **bank-relative value geometry directly**, while keeping the accepted dual-tangent supervision fixed and excluding the already-observed C2 outer holdout from further adaptive development.
...
- Fresh/final benchmark sets must remain isolated from model/hyperparameter selection. **The official LOL-v2 Real test remains untouched through T059-D.**
...
- **Matched-detail source-fit candidate:** T059-BF/T059-C2 dual-tangent line. In-source fitting succeeds, but the fixed image-held-out generalization gate fails; T059-D further rejects additive-offset-only rescue. This line is **not deployable** and cannot proceed to real-domain detail rollout without a new scientifically justified generalization fix and fresh validation.
...
PR #75/T050 is merged as `276c0b1fa5c9e6548bef90048ec6fcc43da439c3`. PRs #76–#83 inherit evidence-history/integration complications; preserve exact accepted scientific/evidence states rather than rewriting history during experiment cycles. PR #83/T057 may remain non-mergeable for history reasons; that does not invalidate its reviewed scientific evidence. PR #95/T058-AF, PR #99/T059-BF, PR #101/T059-C2, and PR #102/T059-D are likewise scientifically reviewable despite inherited history; do not repair history inside experiment cycles.

## Current open task

**T059-E — bank-relative value-loss nested source-transfer test** in `coordination/CHATGPT_TO_CODEX.md`.

Do not touch the already-observed T059-C2 outer holdout (`image_group_index % 5 == 0`). On the remaining 64 source images, use one fixed nested split: `%5==1` as 16-image inner holdout and `%5 in {2,3,4}` as 48-image inner training. Keep the 28-D EnergyHead and legacy/detail Sobolev supervision fixed, but replace only the pooled absolute value term with a deterministic `state_index==0` within-bank relative Huber objective. Train exactly once with the frozen seed/optimizer/epoch recipe, persist/hash the checkpoint before inner-held-out supervision opens, and judge only the preregistered bank-relative value plus legacy/detail gates on the inner holdout. This exploratory task cannot authorize reuse of the observed outer C2 holdout, target-domain TTT, LOL-v2, official-test access, PSNR/SSIM selection, clean/reference information at test time, or real-domain detail rollout.
